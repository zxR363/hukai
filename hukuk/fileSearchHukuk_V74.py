...
# ==================================================
# 1️⃣ KONFİGÜRASYON SINIFI (V74: SAME CONFIG)
# ==================================================
@dataclass
class LegalConfig:
    SOURCES = {
...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V74: DRIFT & CONFLICT AWARENESS)
# ==================================================
class LegalMemoryManager:
    ...
    # --- V74 YENİ: POLARITY (YÖN) TESPİTİ ---
    def _detect_polarity(self, principle_text):
        prompt = f"""
Aşağıdaki hukuki ilke davacı/talep eden açısından mı (LEHINE) yoksa davalı/reddeden açısından mı (ALEYHINE) yorumlanmıştır?

İLKE: "{principle_text}"

CEVAP (SADECE BİRİ):
[LEHINE] veya [ALEYHINE]
"""
        try:
    ...
    # --- V74 GÜNCELLEMESİ: RECALL ILE UYARI VERME ---
    def recall_principles(self, query_text):
        try:
            ...
            for h in hits:
                ...
                if final_conf > 0.20:
                    item = {
                        ...
                        "conflict": h.payload.get("conflict_flag", False)  # Çelişkiyi al
                    }
            ...
            for item in sorted_hits:
                # Çelişki Uyarısı Ekle
                warning = "⚠️ [YARGISAL ÇELİŞKİ VAR]" if item["conflict"] else ""
    ...
    # --- V74 YENİ: GELİŞMİŞ İLKE KAYDI (TREND + ÇELİŞKİ) ---
    def _save_principle_v73(self, text, confidence, source_ids, domain, cluster_data):
        """
        V73: Polarity, Trend ve Çelişki Analizi ile Kayıt
        """
        try:
            vec = self.embedder.embed_query(text)
            polarity = self._detect_polarity(text)  # Yön tespiti

            # Benzer ilkeleri çek (Çelişki kontrolü için)
            hits = self.client.query_points(
                collection_name=LegalConfig.MEMORY_COLLECTIONS["principle"],
                query=vec,
                limit=5,
            ...
            # 1. Çelişki Kontrolü
            for h in hits:
                old_polarity = h.payload.get("polarity")
                if old_polarity and old_polarity != polarity and old_polarity != "BELIRSIZ":
                    conflict = True  # Zıt yönlü karar bulundu!

            # 2. Trend Analizi
            for c in cluster_data:
                ts = c.get("timestamp", time.time())
                bucket = self._extract_year_bucket(ts)
                decision = c.get("decision", "KABUL")  # Varsayılan Kabul
                trend[(bucket, decision)] += 1

            # Counter'ı JSON formatına çevir
            trend_dict = {}
            ...
            payload = {
                ...
                "domain": domain,
                "polarity": polarity,  # V74
                "trend": trend_dict,  # V74
                "conflict_flag": conflict,  # V74
                "source_count": len(source_ids),
                "source_ids": source_ids,
                "generated_by": "consolidation_v73",
                "timestamp": time.time()
            }
            ...
            conflict_msg = "⚠️ ÇELİŞKİLİ" if conflict else "✅ TUTARLI"
            print(f"   🏆 YENİ İÇTİHAT ({conflict_msg}): Trend={len(trend)} dönem, Yön={polarity}")

        except Exception as e:
            print(f"⚠️ İlke kaydetme hatası: {e}")

    # --- V74: KONSOLİDASYON GÜNCELLEMESİ ---
    def consolidate_principles_v73(self):
        print("\n🔥 İÇTİHAT MİMARI: İlke-Temelli Konsolidasyon (V74: Drift & Conflict)...")
        try:
            points, _ = self.client.scroll(
                collection_name=LegalConfig.MEMORY_COLLECTIONS["decision"],
                limit=150
            ...
            for p in points:
                if (p.payload.get('doc_type') == 'EMSAL KARAR' and
                        len(p.payload.get('reason',
                                          '')) > 30):  # Red kararlarını da trend için alabiliriz ama şimdilik odak Kabul

                    if p.payload.get('decision') == 'KABUL':
                        candidates.append({
                            "reason": p.payload['reason'],
                            "id": p.id,
                            "timestamp": p.payload.get('timestamp', time.time()),  # V74: Zaman damgası şart
...
"""
                ...
                if principle_match:
                    ...
                    # V74: Yeni Kayıt Fonksiyonunu Çağır (Cluster datasını da gönder)
                    self._save_principle_v73(principle_text, conf, source_ids, domain_text, cluster)
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V74: Drift & Conflict Awareness):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V74: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V74.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V74 (Judicial Drift & Conflict Awareness)...")
    ...
    def run(self):
        ...
        # Gelişmiş Konsolidasyon (V74: Drift & Conflict)
        if self.memory_manager:
            self.memory_manager.consolidate_principles_v73()
