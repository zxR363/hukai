...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V82: EXPLAINABLE AI)
# ==================================================
...
"""
    ...
    def _calculate_case_success_probability(self, principle_confidence, trend_direction, conflict, domain_match):
        ...
        # --- SÖZEL GEREKÇELENDİRME (EXPLAINABILITY) ---
        key_factors = []
        risk_factors = []
        ...
        # 3. Çelişki Durumu
        if conflict:
            risk_factors.append("Kararlarda çelişkiler bulunmaktadır (Risk)")
        else:
    ...
    def recall_principles(self, query_text):
        try:
            ...
            for item in sorted_hits:
                # V82: DETAYLI AÇIKLAMA MOTORUNU ÇAĞIR
                ...
                # --- LOGLAMA (Sadece sorgu değiştiyse) ---
                if self.last_recalled_query != query_text:
                    print("\n" + "-" * 60)
                    print(f"📊 [RİSK & BAŞARI ANALİZİ] (V82: Explainable AI)")
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V82: Explainable AI)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v82",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V82: Explainable AI)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V82: Explainable AI):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V82: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V82.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V82 (Explainable AI - Why %72?)...")
