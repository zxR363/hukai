...
from langchain_core.messages import SystemMessage, HumanMessage  # <-- YENİ: Mesaj Mimarisi İçin Gerekli
...
SOURCES = {
    "mevzuat": {
        "folder": "mevzuatlar",
        "collection": "legal_statutes_v48",  # Koleksiyonları koruduk
        "desc": "MEVZUAT"
    },
    "emsal": {
        "folder": "belgeler",
        "collection": "legal_precedents_v48",  # Koleksiyonları koruduk
        "desc": "EMSAL KARAR"
    }
}
...
def force_unlock_db():
    ...
    if os.path.exists(lock_file):
        try:
            os.remove(lock_file)
            print("🔓 KİLİT DOSYASI SİLİNDİ.")
        except Exception as e:
            print(f"⚠️ Kilit silinemedi: {e}")


# ==================================================
# 2️⃣ PDF SONU OKUYUCU
# ==================================================
...
def check_relevance_judge_smart(llm, user_query, user_filter, negative_keywords, document_text, source_name):
    ...
    scope = f"Odak: {user_filter}" if user_filter else "Genel"
    #prompt_gen = f"HUKUKÇU. Sorgu: '{user_query}' {scope}. Belge: '{document_text[:600]}...'. Delil olur mu? [EVET]/[HAYIR] ve sebep."
    prompt_gen = f"""
        SEN KIDEMLI BIR HUKUKCUSSUN.
    
        SORGUNUN AMACI:
        Benzer Yargıtay içtihatlarını ve hukuki ilke kararlarını bulmak.
        ...
        - hukuki ilke,
        - yorum yaklaşımı,
        - miras hukuku mantığı
    
        bakımından sorguyla ne derece BENZER?
    ...
    is_ok = ("ÇOK BENZER" in res.upper()) or ("BENZER" in res.upper())
...
def run_indexing_v53():
    ...
    try:
        ...
        print(f"\n❌ VERİTABANI HATASI: {e}")
        print("💡 ÇÖZÜM: 'qdrant_db_master' klasörünü silip tekrar başlatın.")
        return False
...
def create_pdf_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V53.pdf"):
    pdf = LegalReport();
    pdf.add_page();
...
# ==================================================
# 6️⃣ ANA MOTOR (V53: SYSTEM/HUMAN MESSAGE ARCHITECTURE)
# ==================================================
def main():
    print("🚀 LEGAL SUITE V53 (Architecture Fix: No Echo, Full Iterator)...")

    if not run_indexing_v53(): sys.exit()
    ...
    while True:
        ...
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (Iterator Mode - Fixed Architecture)...")

        # --- V53: AYRIŞTIRILMIŞ MESAJ YAPISI (System vs Human) ---
        # Prompt metni V49 ile KELİMESİ KELİMESİNE AYNIDIR, sadece ikiye bölündü.

        # 1. KISIM: KURALLAR (Sistem Mesajı)
        system_content = """SEN KIDEMLİ BİR HUKUKÇUSUN.

⚠️ ANALİZ YÖNTEMİ (ITERATOR MODE):

1. **BELGE BELGE GİT:**
   - Deliller listesindeki Belge #1'den başla, sırayla git.
   - **ASLA AYNI DOSYAYI TEKRAR ETME.** Sadece yeni bilgi varsa ekle.

2. **KAYNAK DİSİPLİNİ:**
   - Sadece "DELİLLER" kısmında yazan "DOSYA ADI" bilgisini kullan. 
   - Listede olmayan bir dosyayı (örn: hayali bir dosya) ASLA yazma.
...
3. **FORMAT:**
   ...
   C. SONUÇ VE TAVSİYE
      - Sadece yukarıdaki delillere dayanarak tavsiye ver."""

        # 2. KISIM: VERİLER (Kullanıcı Mesajı)
...
ANALİZİ BAŞLAT:"""
        ...
        # Mesajları Liste Olarak Gönderiyoruz (Teknik Değişiklik)
        messages = [
