...
import atexit
import json
import math  # Vektör hesaplamaları için eklendi
...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V71: JURISPRUDENCE ENGINE)
# ==================================================
class LegalMemoryManager:
    ...
    def recall_principles(self, query_text):
        try:
            ...
            # V71: Güven skoruna (confidence) göre sırala
            sorted_hits = sorted(hits, key=lambda x: (
                x.payload.get("confidence", 0.5),  # Güven yoksa varsayılan 0.5
                x.payload.get("timestamp", 0)
            ), reverse=True)[:3]

            if not sorted_hits: return ""

            memory_text = "\n💡 YERLEŞİK İÇTİHAT HAFIZASI (Sistem Tarafından Üretildi):\n"
            for h in sorted_hits:
                conf = h.payload.get('confidence', 0.0) * 100
                memory_text += f"- {h.payload['principle']} (Güven: %{conf:.0f})\n"
            return memory_text
        except:
            return ""
    ...
    # --- V71: MATEMATİKSEL KÜMELEME YARDIMCISI ---
    def _cosine_similarity(self, v1, v2):
        """İki vektör arasındaki Cosine Similarity'yi hesaplar (Pure Python)."""
    ...
    def _cluster_reasonings(self, items, threshold=0.86):
        """
        Gerekçeleri benzerliklerine göre kümeler.
        items:List[dict] -> {'text': str, 'vector': list}
        """
        clusters = []
        for item in items:
            added_to_cluster = False
            for cluster in clusters:
                # Kümenin merkezi (ilk elemanı) ile karşılaştır
                centroid = cluster[0]['vector']
                sim = self._cosine_similarity(item['vector'], centroid)
    ...
    def _calculate_principle_confidence(self, cluster):
        """
        Güven = (Sayı Skoru * 0.6) + (Benzerlik Skoru * 0.4)
        """
        count = len(cluster)
        # 10 adet teyit varsa tam güven (1.0)
        count_score = min(1.0, count / 10)

        # Küme içi tutarlılık (similarity)
        if count > 1:
            centroid = cluster[0]['vector']
            sims = [self._cosine_similarity(c['vector'], centroid) for c in cluster[1:]]
            similarity_score = sum(sims) / len(sims)
        else:
            similarity_score = 1.0  # Tek eleman kendine benzer

        return round((count_score * 0.6) + (similarity_score * 0.4), 2)

    # --- V71: İLKE-TEMELLİ KONSOLİDASYON (ANA FONKSİYON) ---
    def consolidate_principles_v71(self):
        """
        Aynı gerekçeye sahip kararları grupla -> ortak ilkeyi LLM ile özetle -> güven ile kaydet.
        """
        print("\n🔥 İÇTİHAT MOTORU: İlke-Temelli Konsolidasyon Başlatılıyor (V71)...")
        try:
            # 1. Son kararları çek (Emsal Karar ve Kabul olanlar)
            points, _ = self.client.scroll(
                collection_name=LegalConfig.MEMORY_COLLECTIONS["decision"],
                limit=100
            )

            # Filtrele: Sadece yeterince uzun gerekçesi olan EMSAL KARARLAR
            candidates = []
            for p in points:
            ...
            if len(candidates) < 3:
                print("   ℹ️ Yeterli veri birikmedi (En az 3 emsal gerekçesi gerekli).")
                return

            print(f"   🔍 {len(candidates)} adet nitelikli gerekçe analiz ediliyor...")

            # 2. Gerekçeleri Vektörleştir (Kümeleme için)
            ...
            for txt, vec in zip(texts, vectors):
                clustered_data.append({"text": txt, "vector": vec})

            # 3. Kümeleme
            clusters = self._cluster_reasonings(clustered_data, threshold=0.86)
            print(f"   🧩 {len(clusters)} farklı hukuki yaklaşım kümesi bulundu.")

            # 4. Her küme için İlke Üretimi
            for i, cluster in enumerate(clusters):
                if len(cluster) < 3: continue  # Gürültüyü engelle, en az 3 teyit lazım

                print(f"   ⚙️ Küme #{i + 1} ({len(cluster)} karar) içtihatlaştırılıyor...")

                # LLM'e gönderilecek gerekçeler
                reasonings_text = "\n".join([f"- {c['text']}" for c in cluster])

                prompt = f"""
Aşağıdaki yargısal gerekçeler aynı hukuki yaklaşıma sahiptir.
Bu gerekçelerden TEK bir soyut hukuki ilke üret.

Kurallar:
- Genel geçer olmalı
- Şartlı cümle yapısı içermeli
- Yargıtay diliyle yazılmalı
- Tek paragraf

Gerekçeler:
{reasonings_text}
"""
                principle = self.llm.invoke(prompt).content.strip()

                # 5. Güven Hesabı
                confidence = self._calculate_principle_confidence(cluster)

                # 6. Kaydet
                self._save_principle_v71(principle, confidence, len(cluster))

            print("✅ İçtihat üretimi tamamlandı.")

        except Exception as e:
            print(f"⚠️ Konsolidasyon hatası: {e}")

    def _save_principle_v71(self, text, confidence, count):
        try:
            ...
            self.client.upsert(
                ...
                points=[PointStruct(
                    ...
                    payload={
                        ...
                        "source_count": count,
                        "generated_by": "principle_consolidation_v71",
                        "timestamp": time.time()
                    }
                )]
            )
            print(f"   🏆 YENİ İÇTİHAT KAYDEDİLDİ (Güven: {confidence}):\n      '{text[:100]}...'")
        except:
            pass
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V71: Jurisprudence Engine):")
        valid_docs = []
        ...
        for hit in candidates:
            ...
            # Konsensüs Skoru (Keşif Bonusu Dahil)
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V71: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V71.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V71 (Jurisprudence Generator)...")
    ...
    def run(self):
        ...
        # V71 YENİLİĞİ: GÜNLÜK İLKE ÜRETİMİ (JURISPRUDENCE)
        if self.memory_manager:
            self.memory_manager.consolidate_principles_v71()
