...
class LegalUIPrinter:
    ...
    def print_grand_ui_log(ui_data, doc_scan_log):
        ...
        print("\n" + "█" * 80)
        print(f"🖥️  LEGAL OS V101 - TAM KAPSAMLI ANALİZ VE TAKİP RAPORU (UI DATA)")
        print("█" * 80 + "\n")
...
class LegalSearchEngine:
    ...
    def retrieve_raw_candidates(self, full_query):
        ...
        # Kota uygula: 7 Emsal + 3 Mevzuat (Zorunlu)
...
"""
    ...
    def _check_relevance_judge_smart(self, user_query, user_filter, negative_keywords, document_text, source_name):
        ...
        # V101: YARGIÇ PROMPTU GÜNCELLEMESİ (TÜR FARKINDALIĞI)
        prompt_gen = f"""
SEN KIDEMLI BIR HUKUKCUSSUN.
{memory_context}

SORGUNUN AMACI: İlgili hukuki dayanakları (Kanun veya Emsal) bulmak.
Sorgu: "{user_query}"
Belge: "{document_text[:700]}..."

GÖREV:
EĞER BELGE BİR KANUN MADDESİ (MEVZUAT) İSE:
- Bu madde, yukarıdaki olaya hukuki dayanak teşkil ediyor mu?
- Evet ise [KABUL] ver.

EĞER BELGE BİR MAHKEME KARARI (EMSAL) İSE:
- Bu karar, olay örgüsü bakımından benzer mi?
- Evet ise [KABUL] ver.
...
"""
        ...
        is_ok = ("ÇOK BENZER" in res) or ("BENZER" in res) or ("KABUL" in res)
        return is_ok, res
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V101: Balanced Jurist):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V101: Final Output)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V101.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V101 (The Balanced Jurist: Statute Aware)...")
