...
class LegalUIPrinter:
    ...
    def print_grand_ui_log(ui_data, doc_scan_log):
        ...
        print("\n" + "█" * 80)
        print(f"🖥️  LEGAL OS V105 - KURUMSAL ANALİZ, AUDIT VE RAPORLAMA")
        print("█" * 80 + "\n")
...
# ==================================================
# 9️⃣ V105: YENİ KURUMSAL RAPORLAMA ARAÇLARI
# ==================================================
class AuditTimelineBuilder:
    """Audit logları işleyerek skor değişim zaman çizelgesi oluşturur"""
    ...
    def build(audit_logs):
        ...
        # 'audit_logs'un dict mi list mi olduğunu kontrol et
        ...
        for idx, log in enumerate(logs_list):
            ...
            # Bazı loglarda skor olmayabilir (örn: sadece bilgi mesajı), onları atla
            if score is None: continue

            # Delta (Değişim) hesabı
            delta = None
            if last_score is not None:
                delta = round(score - last_score, 1)

            timeline.append({
                "step": idx + 1,
                "stage": log.get("title", "İşlem"),
                "score": score,
                "delta": delta
            })
            last_score = score
...
class ScoreExplanationEngine:
    """Zaman çizelgesine bakarak değişimi insan diliyle açıklar"""
    ...
    def generate(timeline):
        ...
        if decreases:
            ...
            parts.append(
                f"Başarı olasılığı, '{worst['stage']}' aşamasında riskler nedeniyle %{abs(worst['delta'])} düşmüştür.")

        if increases:
            best = max(increases, key=lambda x: x["delta"])
            parts.append(
                f"Ancak '{best['stage']}' aşamasında yapılan stratejik değerlendirme skoru %{best['delta']} artırmıştır.")

        if not parts:
            return "Analiz süresince başarı skoru durağan bir seyir izlemiştir."

        return " ".join(parts)


class CorporateCover:
    """PDF için kurumsal kapak sayfası üreticisi"""

    @staticmethod
    def add(pdf, case_id, version="V105"):
        pdf.add_page()

        # Logo placeholder (Metin olarak)
        ...
        pdf.multi_cell(0, 5,
                       "YASAL UYARI: Bu rapor, yapay zeka algoritmalari kullanilarak uretilmistir. Hukuki tavsiye niteliginde olmayip, karar destek amaclidir. Nihai hukuki degerlendirme yetkili bir avukat tarafindan yapilmalidir.",
                       align="C")


# ==================================================
# 🔟 RAPORLAMA SINIFI (REPORTER - V105 UPDATED)
# ==================================================
class PDFReportGenerator(FPDF):
    def header(self):
        # Kapak sayfasında header olmasın (Sayfa 1)
        if self.page_no() > 1:
            self.set_font('helvetica', 'B', 10)
            self.cell(0, 10, 'LEGAL OS - HUKUKI ANALIZ RAPORU', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='R')
...
class LegalReporter:
    ...
    def add_audit_log_section(pdf, audit_data):
        ...
        pdf.cell(0, 10, "4. KARAR SÜRECİ VE DENETİM (AUDIT LOG)", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(3)

        # V105: Skor Değişim Özeti (Explanation Engine)
    ...
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, audit_data=None, filename="Hukuki_Rapor_V105.pdf"):
        pdf = PDFReportGenerator();

        # V105: Kurumsal Kapak
        case_id = audit_data.get("case_id", "BILINMIYOR") if audit_data else "N/A"
        CorporateCover.add(pdf, case_id)

        # Rapor İçeriği (Sayfa 2'den başlar)
        pdf.add_page();
...
# ==================================================
# 1️⃣1️⃣ ANA UYGULAMA (MAIN APP)
# ==================================================
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V105 (Corporate Intelligence Edition)...")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                self.reporter.create_report(story, valid_docs, full_advice, audit_dump, "Hukuki_Rapor_V105.pdf")
