...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V93: SIMULATION & MITIGATION ENGINE)
# ==================================================
class LegalMemoryManager:
    # --- V93: SIMULATION CONFIGURATION ---
    MITIGATION_EFFECTS = {
        "DELIL": {"min": 5, "max": 10},  # Güçlü delil çok puan getirir
        "BELGE": {"min": 5, "max": 10},
        "ICTIHAT": {"min": 3, "max": 7},  # İyi emsal orta puan
        "ARGUMAN": {"min": 3, "max": 7},
        "TALEP_DARALTMA": {"min": 4, "max": 6},  # Stratejik hamle
        "USUL": {"min": 2, "max": 4}  # Usul düzeltmeleri az puan
    }
    MAX_TOTAL_BOOST = 15  # Toplam artış asla 15'i geçemez (Gerçekçilik sınırı)
    MAX_SCORE = 95  # Hiçbir dava %100 değildir
...
⚠️ Kurallar: “Bu nedenle”, “dosya kapsamı”, “mahkemenin kanaati” ifadeleri kullan. İçtihat atfı yapma.
...
⚠️ Kurallar: Sayısal skorları gerekçeye bağla. Hakimin tereddüdünü vurgula.
...
"""
    ...
    # --- V93 YENİ: STRATEJİK GÜÇLENDİRME & SİMÜLASYON ---
    def _estimate_mitigation_impact(self, rec_text, min_val, max_val):
        """
        LLM ile önerinin potansiyel etkisini puanlar.
        """
        prompt = f"""
Aşağıdaki hukuki güçlendirme önerisinin dava başarısına etkisini tahmin et.

ÖNERİ: "{rec_text}"

Kurallar:
- Etki {min_val} ile {max_val} puan arasında olmalı
- Aşırı iyimser olma
- Sadece bir sayı (integer) döndür

CEVAP FORMATI (SADECE SAYI):
"""
        try:
            res = self.llm.invoke(prompt).content.strip()
            val = int(re.findall(r"\d+", res)[0])
            return max(min(val, max_val), min_val)  # Clamp logic
        except:
            return min_val  # Hata durumunda minimum etki

    def _simulate_post_strengthening_score(self, base_score, recommendations):
        """
        Önerilen iyileştirmelerin toplam etkisini hesaplar ve yeni skoru simüle eder.
        """
        total_boost = 0
        drivers = []

        for rec in recommendations:
            rec_type = rec["focus"].upper()  # V92'den gelen focus (delil, belge...)
            rec_text = rec["suggestion"]

            # Mapping V92 focus to V93 Config keys
            config_key = "DELIL"  # Default
            if "DELIL" in rec_type or "TANIK" in rec_type:
                config_key = "DELIL"
            elif "BELGE" in rec_type:
                config_key = "BELGE"
            elif "ARGUMAN" in rec_type or "ICTIHAT" in rec_type:
                config_key = "ARGUMAN"
            elif "USUL" in rec_type:
                config_key = "USUL"
            elif "TALEP" in rec_type:
                config_key = "TALEP_DARALTMA"

            cfg = self.MITIGATION_EFFECTS.get(config_key, {"min": 1, "max": 3})

            impact = self._estimate_mitigation_impact(rec_text, cfg["min"], cfg["max"])
            total_boost += impact
            drivers.append(f"{config_key}(+{impact})")
...
GÖREVİN: “Bu dosya nasıl güçlendirilebilir?” sorusuna, uygulanabilir ve somut öneriler üret.

⚠️ Kurallar:
- Önerileri JSON formatında ver.
- Her öneri bir zayıf noktaya karşılık gelsin.
- TÜR (focus) olarak şunlardan birini seç: DELIL, BELGE, ARGUMAN, USUL, TALEP_DARALTMA.

---
⚖️ HAKİM TEREDDÜT NOKTALARI: {judge_uncertainties}
❌ OLASI RED GEREKÇESİ: {counter_rejection}
📊 TREND UYUMSUZLUKLARI: {trend_conflicts}
📉 ZAYIF İLKELER: {low_confidence_principles}
...
"""
        try:
            ...
            return {"summary": "Hata", "recommendations": []}
    ...
    def recall_principles(self, query_text):
        try:
            ...
            for item in sorted_hits:
                ...
                # 3. Güçlendirme Önerileri
                strengthening_plan = self._generate_strengthening_recommendations(
                    judicial_text, rejection_text, item['evolution_note'], f"Güven: {item['conf']:.2f}"
                )

                # 4. V93: SİMÜLASYON MOTORU (YENİ SKOR HESABI)
                simulation_result = self._simulate_post_strengthening_score(
                    analysis['success_probability'],
                    strengthening_plan.get('recommendations', [])
                )

                if self.last_recalled_query != query_text:
                    print("\n" + "=" * 70)
                    print(f"📊 [MAHKEME SALONU SİMÜLASYONU] (V93: Simulation Engine)")
                    print(f"   🎯 KONU: {query_text} ({query_domain})")
                    print("-" * 70)
                    print(f"📝 YÖNETİCİ ÖZETİ: \"{exec_summary[:150]}...\"")
                    print("-" * 70)
                    print(f"⚖️  MEVCUT EĞİLİM: {net_decision['decision']} ({net_decision['final_score']})")
                    print(f"   🎲 Mevcut Başarı İhtimali: %{analysis['success_probability']}")
                    print("-" * 70)
                    print("💡 GÜÇLENDİRME ÖNERİLERİ:")
                    for rec in strengthening_plan.get('recommendations', []):
                        print(f"   🔧 [{rec['focus']}] {rec['suggestion']}")
                    print("-" * 70)

                    # V93 YENİ LOG: SİMÜLASYON SONUCU
                    print(f"🚀 SİMÜLASYON SONUCU (ETKİ ANALİZİ):")
                    print(
                        f"   📊 Skor Değişimi: %{simulation_result['current_score']} -> %{simulation_result['projected_score']}")
                    print(f"   📈 Toplam Artış: +{simulation_result['total_boost']} Puan")
                    print(f"   🔑 Ana Etkenler: {', '.join(simulation_result['drivers'])}")
                ...
                memory_text += f"  🏆 MEVCUT DURUM: {net_decision['decision']} (Skor: %{analysis['success_probability']})\n"
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V93)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v93",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V93: Simulation Engine)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V93: Simulation Engine):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V93: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V93.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V93 (The Future Simulator: Mitigation Engine)...")
