...
class QueryContext:
    ...
    def __init__(self, reporters):
        self.detected_domain = self.detect_domain()
    ...
    def detect_domain(self):
        # Sorgudan domain algıla (basit kelime matching)
        text_lower = self.query_text.lower()
        if any(word in text_lower for word in ["miras", "veraset", "iskat", "vasiyet"]):
            return "miras_hukuku"
        elif any(word in text_lower for word in ["borc", "alacak", "tahsil", "icra"]):
            return "borclar_hukuku"
        elif any(word in text_lower for word in ["bosanma", "nafaka", "velayet", "mal paylasimi"]):
            return "aile_hukuku"
        elif any(word in text_lower for word in ["ceza", "suç", "mahkumiyet", "beraat"]):
            return "ceza_hukuku"
        else:
            return "genel_hukuk"  # Fallback
