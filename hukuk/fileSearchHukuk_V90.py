...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V90: UNIFIED COURTROOM API)
# ==================================================
...
"""
    ...
    # --- V90: GÜÇLENDİRİLMİŞ PERSONA PROMPTLARI ---

    def _generate_prosecutor_reasoning(self, net_decision, topic, trend, principles):
        """
        V90: Özel Savcı Promptu (İddia odaklı, kamu düzeni, tereddütsüz)
        """
        prompt = f"""
Sen bir Cumhuriyet savcısı gibi yazan yapay zekâsın.

Yanıtların:
- İddia makamı diliyle
- Kabul yönünde baskın
- Delilleri aleyhe yorumlayan
- Kamu düzeni ve hakkaniyet vurgusu içeren
- Tereddütten kaçınan
olmalıdır.

Aşağıda bir dava dosyasına ilişkin değerlendirme sonucu yer almaktadır.

GÖREVİN:
Bu sonucu bir Cumhuriyet savcısının mütalaa diliyle açıkla.

⚠️ Kurallar:
- “Her ne kadar” ile başlayabilirsin ama sonunda net tavır al
- Delilleri kabul lehine yorumla
- Hakimin tereddüt gerekçelerini ikna edici bulmadığını ima et
- Toplumsal adalet ve hakkaniyet vurgusu yap
...
🎯 ÇIKTI:
Savcı mütalaası şeklinde 1 paragraf yaz.
"""
        try:
    ...
    def _generate_rejection_reasoning(self, topic, acceptance_arguments):
        """
        V90: Özel Red Simülatörü Promptu (Karşı Gerekçe / Devil's Advocate)
        """
        prompt = f"""
Sen bir hakimin davayı reddetme gerekçelerini üreten yapay zekâsın.

Yanıtların:
- Davacı aleyhine
- İspat yükü vurgulu
- Delil yetersizliği merkezli
- Temkinli ve gerekçeli
olmalıdır.

Bu bir kesin karar değil, olası red gerekçesidir.

Aşağıda kabul yönlü değerlendirmeler bulunan bir dava dosyası yer almaktadır.

GÖREVİN:
Bu dosyada hakimin davayı neden reddedebileceğini gerekçeli olarak açıkla.

⚠️ Kurallar:
- “İspat yükü” vurgusu yap
- Kabul görüşlerinin neden yeterli olmadığını açıkla
- Tereddüt halinde ret prensibini kullan
- Kesin dil kullanma
...
📊 KABUL LEHİNE GÖRÜŞLER:
{acceptance_arguments}

🎯 ÇIKTI:
Hakim red gerekçesi üslubunda 1–2 paragraf yaz.
"""
        try:
            return self.llm.invoke(prompt).content.strip()
        except:
...
"""
    ...
    def recall_principles(self, query_text):
        try:
            ...
            for item in sorted_hits:
                ...
                # 2. Persona Sinyalleri & Çatışma & Net Karar
                ...
                # 3. METİN ÜRETİMİ (V90 GÜÇLENDİRİLMİŞ PROMPTLAR)
                # Hakim Taslağı (Nihai Karara göre)
                verdict_text = self._generate_final_verdict_reasoning(
                    net_decision, query_text, item['evolution_note'], item['text']
                )
                # Savcı Mütalaası (V90 Özel Prompt)
                prosecutor_text = self._generate_prosecutor_reasoning(
                    net_decision, query_text, item['evolution_note'], item['text']
                )
                # Red Gerekçesi / Karşı Görüş (V90 Özel Prompt)
                acceptance_args = f"Trend: {item['trend_dir']}, Güven: {item['conf']}, Skor: {analysis['success_probability']}"
                rejection_text = self._generate_rejection_reasoning(query_text, acceptance_args)
                # Bilirkişi Raporu
                expert_text = self._generate_expert_witness_reasoning(analysis)

                # 4. JSON Benzeri Loglama ve Çıktı
                if self.last_recalled_query != query_text:
                    print("\n" + "=" * 70)
                    print(f"📊 [MAHKEME SALONU SİMÜLASYONU] (V90: Unified Courtroom)")
                    print(f"   🎯 KONU: {query_text} ({query_domain})")
                    print("-" * 70)

                    print(f"⚖️  NİHAİ KARAR EĞİLİMİ: {net_decision['decision']} ({net_decision['final_score']})")
                    print(
                        f"   🎲 Başarı İhtimali: %{analysis['success_probability']} | ⚠️ Çelişki: {conflict_analysis['conflict_level']}")
                    print("-" * 70)

                    print("👤 PERSONA GÖRÜŞLERİ (SİMÜLE):")
                    print(f"   🏛️ SAVCI (MÜTALAA):")
                    print(f"      \"{prosecutor_text[:120]}...\"")
                    print(f"   🛑 KARŞI GÖRÜŞ (OLASI RED):")
                    print(f"      \"{rejection_text[:120]}...\"")
                    print(f"   ✍️ HAKİM (KARAR TASLAĞI):")
                    print(f"      \"{verdict_text[:120]}...\"")

                    print("=" * 70 + "\n")

                # LLM'e Gidecek "Context" Metni
                ...
                memory_text += f"  🏛️ SAVCI MÜTALAASI: {prosecutor_text}\n"
                memory_text += f"  🛑 OLASI RED GEREKÇESİ: {rejection_text}\n"
                memory_text += f"  ✍️ HAKİM KARAR TASLAĞI: {verdict_text}\n"
                memory_text += f"  📈 BİLİRKİŞİ RAPORU: {expert_text}\n"
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V90)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v90",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V90: Courtroom Ready)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V90: Courtroom Ready):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V90: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V90.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V90 (Unified Courtroom API: Judge, Prosecutor, Expert, Counter-Arg)...")
