...
class PersonaEngine:
    ...
    def run(self, context: QueryContext, decision_context: DecisionContext, judge_reflex: JudgeReflex) -> List[
        PersonaResponse]:
        ...
        roles = [
            ("DAVACI VEKİLİ", "Hakimin tereddütlerini gider, davanın kabulü için argüman üret."),
            ("DAVALI VEKİLİ", "Hakimin tereddütlerini derinleştir, davanın reddi için itiraz et."),
            ("BİLİRKİŞİ", "Tereddütlerin hukuki tutarlılığını ve delil zincirini denetle.")
        ]
    ...
    def validate_madde_from_source(self, kanun_kodu: str, madde_range: str,
                                   mapping: dict) -> str:  # mapping parametresi ekle
        validated_maddeler = f"{kanun_kodu} {madde_range} (Doğrulanmış metinler: "  # Fallback başlangıç

        # Adım 1: Yerel PDF varsa öncelikli oku
        pdf_path = os.path.join(LegalConfig.SOURCES["mevzuat"]["folder"], f"{kanun_kodu}.pdf")  # Örnek: TMK.pdf
        if os.path.exists(pdf_path):
            try:
                ...
                doc.close()
                if found_text:
                    validated_maddeler += found_text[:1000] + "..."  # Kısalt
                    return validated_maddeler + ")"
            except Exception as e:
                print(f"Yerel PDF hatası: {e}")

        # Adım 2: Yerel yoksa web URL'i kullan (mapping'ten otomatik çek)
        url = mapping.get("pdf_url", "")  # Domain'e göre URL
        if url:
            try:
    ...
    def generate_domain_focus(self, ctx: QueryContext, DOMAIN_MAPPINGS=None) -> str:
        ...
        # Doğrulama çağrısı (mapping'i geçir)
...
"""
    ...
    def _invoke_persona(self, role: str, instruction: str, context: QueryContext) -> PersonaResponse:
        # DOMAIN'E GÖRE OTOMATİK MADDE VE İÇTİHAT ZORLAMASI
        DOMAIN_MAPPINGS = {
            "miras_hukuku": {
                ...
                "pdf_url": "https://mevzuat.gov.tr/mevzuatmetin/1.5.4721.pdf"  # TMK PDF
            },
            "borclar_hukuku": {
                ...
                "pdf_url": "http://www.mevzuat.gov.tr/MevzuatMetin/1.5.6098.pdf"  # TBK PDF
            },
            "aile_hukuku": {
                ...
                "pdf_url": "https://mevzuat.gov.tr/mevzuatmetin/1.5.4721.pdf"  # TMK (aile de TMK altında)
            },
            "ceza_hukuku": {
                ...
                "pdf_url": "http://www.mevzuat.gov.tr/MevzuatMetin/1.5.5237.pdf"  # TCK PDF
            },
            "icra_hukuku": {
                ...
                # İİK PDF (doğrulanmış genel link, gerekirse güncelle)
            },
            "is_hukuku": {
                "maddeler": "İşK md. 1-75 (iş sözleşmesi, kıdem, ihbar)",
                "ictihatlar": "Yargıtay 9. Hukuk Dairesi iş hukuku kararları",
                "pdf_url": "http://www.mevzuat.gov.tr/MevzuatMetin/1.5.4857.pdf"  # İş Kanunu PDF
            },
            "ticaret_hukuku": {
                ...
                "pdf_url": "http://www.mevzuat.gov.tr/mevzuatmetin/1.5.6102.pdf"  # TTK PDF
            },
            "idare_hukuku": {
                ...
                "pdf_url": "http://www.mevzuat.gov.tr/MevzuatMetin/1.5.2577.pdf"  # İYUK PDF
            },
            "vergi_hukuku": {
                ...
                "pdf_url": "https://mevzuat.gov.tr/mevzuatmetin/1.4.213.pdf"  # VUK PDF
            },
            "medeni_usul_hukuku": {
                ...
                "pdf_url": "http://www.mevzuat.gov.tr/MevzuatMetin/1.5.6100.pdf"  # HMK PDF
            },
            "fikri_mulkiyet_hukuku": {
                ...
                "pdf_url": "http://www.mevzuat.gov.tr/mevzuatmetin/1.5.6769.pdf"  # SMK PDF
            },
            "genel_hukuk": {
                "maddeler": "HMK genel hükümleri, TMK/TBK temel ilkeleri",
                "ictihatlar": "İlgili Yargıtay dairesi kararları",
                "pdf_url": "https://mevzuat.gov.tr/mevzuatmetin/1.5.4721.pdf"  # TMK fallback
            }
        }

        # Domain algılama (context'ten al)
...
⚠️ HALÜSİNASYON YASAĞI: Madde numaralarını sadece doğrulanmış metinlerden kullan. Uydurma yapma veya yorum katma.
        ...
        try:
            # LLM çağrısı
            messages = [{"role": "system", "content": "Sen deneyimli bir Türk avukatısın."},
...
"""
    ...
    1. HAKİM EĞİLİMİ: "{judge_reflex.tendency}"
    ...
    ZORUNLU REFERANSLAR ({context.detected_domain.upper()} için):
    - Maddeler: {mapping['maddeler']}
    - İçtihatlar: {mapping['ictihatlar']}

    Cevabında BUNLARI mutlaka kullan. Genel hukuk yazma, doğrudan olaya özel konuş.
