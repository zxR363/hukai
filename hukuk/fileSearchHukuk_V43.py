...
def create_pdf_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V43.pdf"):
    pdf = LegalReport();
    pdf.add_page();
...
# ==================================================
# 5️⃣ ANA MOTOR (V43 ACTIVE CITATION - DÜZELTİLMİŞ)
# ==================================================
def main():
    print("🚀 LEGAL SUITE V43 (Active Citation: Strict Grounding)...")

    if not run_indexing_v43():
        sys.exit()
    ...
    while True:
        ...
        if not candidates:
            print("🔴 Skor eşiğini geçen belge bulunamadı.")
            continue
...
"""
        ...
        # --- DÜZELTİLMİŞ PROMPT (HEM KATI HEM YAPILANDIRILMIŞ) ---
        prompt = f"""
...
1. SINIRLAR (ÇOK ÖNEMLİ):
   ...
   - Asla dışarıdan, kendi bilgilerinden veya hafızandan kanun/karar uydurma (Halüsinasyon görme).
...
2. FORMAT (KESİNLİKLE BU ŞABLONU UYGULA):
   ...
   B. İLGİLİ EMSAL KARARLAR (Yargıtay ve İstinaf Kararları)
      - Deliller arasında [EMSAL KARAR] etiketi taşıyan belgeler varsa, bunları MUTLAKA burada özetle.
      - Emsal kararları görmezden gelme.
