...
# 🔨 Commit 5.6: Persona Engine (Kontrollü LLM - SENIOR & CROSS-CHECK MODE)
class PersonaEngine:
    """
    LLM kontrollü persona simülasyonu.
    Hakimin tereddütlerine cevap üretir.
    Artık Çapraz Doğrulama (Rerank) ve Keskin Bilirkişi moduna sahip.
    """
    ...
    def run(self, context: QueryContext, decision_context: DecisionContext, judge_reflex: JudgeReflex) -> List[PersonaResponse]:
        self.current_doubts = judge_reflex.doubts or ["Dosyanın esasına ilişkin genel delil durumu", "Hukuki tavsif"]

        print(f" 🗣️ Senior Persona Tartışması Başlatılıyor (Çapraz Sorgulu - {len(self.current_doubts)} Tereddüt)...")

        # --- YENİ: KESKİN BİLİRKİŞİ TALİMATI ---
        expert_instruction = """
        GÖREVİN: "Tarafsız Bilirkişi" değil, "YÜKSEK MAHKEME TETKİK HAKİMİ" gibi davran.
        1. Davacı ve Davalı argümanlarını bekleme, doğrudan dosyayı (context) incele.
        2. "Kanaatimce", "takdir mahkemenindir" gibi lafları ASLA KULLANMA.
        3. Dosyadaki hukuki sakatlığı veya eksikliği TEK CÜMLEDE tespit et.
        4. Mevzuat ve İçtihat uyumsuzluğu varsa sert bir dille uyar.
        5. Net ol: "Bu talep hukuka uygundur" veya "Bu talep reddedilmelidir".
        """

        roles = [
            ("DAVACI VEKİLİ", "Hakimin tereddütlerini gider, davanın kabulü için Yargıtay kararı destekli argüman üret."),
            ("DAVALI VEKİLİ", "Hakimin tereddütlerini derinleştir, usul hukukuna dayanarak itiraz et."),
            ("BİLİRKİŞİ", expert_instruction) # Özel sert prompt
        ]
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            future_to_role = {
                executor.submit(self._invoke_persona, role, instruction, context): role
            ...
            for future in as_completed(future_to_role):
                try:
                    ...
                    print(f" ✅ {response.role} beyanı tamamlandı")
                except Exception as e:
                    role = future_to_role[future]
                    print(f" ❌ {role} hatası: {e}")
                    responses.append(PersonaResponse(
                        role=role,
                        response=f"{role}: Beyan oluşturulamadı (hata).",
                        addressed_doubts=self.current_doubts
                    ))

        # --- YENİ: ÇAPRAZ DOĞRULAMA (CROSS-CHECK) ---
        # Personaların beyanlarını alıp mantık süzgecinden geçiriyoruz.
        verified_responses = self._cross_verify_personas(responses, context)

        return verified_responses

    # --- MEVCUT ÖZEL FONKSİYONLARINIZ (AYNEN KORUNDU) ---
    def validate_madde_from_source(self, kanun_kodu: str, madde_range: str, mapping: dict) -> str:
        validated_maddeler = f"{kanun_kodu} {madde_range} (Doğrulanmış metinler: "  # Fallback başlangıç

        # Adım 1: Yerel PDF varsa öncelikli oku
        ...
        if os.path.exists(pdf_path):
            try:
                ...
                for page in doc:
                    text = page.get_text()
                    if re.search(rf"Madde\s+({madde_range.split('-')[0]})", text, re.IGNORECASE):
                        found_text += text.strip() + " (Sayfa: " + str(page.number + 1) + ")\n"
                doc.close()
                if found_text:
                    validated_maddeler += found_text[:1000] + "..."
                    return validated_maddeler + ")"
            except Exception as e:
                print(f"Yerel PDF hatası: {e}")

        # Adım 2: Yerel yoksa web URL'i kullan
        url = mapping.get("pdf_url", "")
        if url:
            try:
                ...
                print(f"Web PDF hatası: {e}")
                validated_maddeler += "Web doğrulaması başarısız – Statik mapping kullanıldı."

        validated_maddeler += ")"
        return validated_maddeler

    def generate_domain_focus(self, ctx: QueryContext, DOMAIN_MAPPINGS=None) -> str:
        domain_key = ctx.detected_domain.lower().replace(" ", "_")
        mapping = DOMAIN_MAPPINGS.get(domain_key, DOMAIN_MAPPINGS["genel_hukuk"])

        tali_konular = ", ".join(ctx.negative_scope) if ctx.negative_scope else "genel tali konular"
        odak_noktalar = re.split(r"[,\n-]", ctx.topic.strip())
        odak_list = "\n- ".join([p.strip() for p in odak_noktalar if p.strip()])

        kanun_kodu = mapping["maddeler"].split()[0] if "md." in mapping["maddeler"] else "ilgili kanun"
        madde_range = mapping["maddeler"].split("md.")[1].strip() if "md." in mapping["maddeler"] else "bilinmiyor"
        ...
        focus_prompt = f"""
⚠️ UYARI:
Bu dosyada {tali_konular} ({kanun_kodu} ilgili maddeler) tali konudur.
Asıl odak:
- {odak_list}

Doğrulanmış Maddeler: {validated_maddeler}
"""
        return focus_prompt

    def _invoke_persona(self, role: str, instruction: str, context: QueryContext) -> PersonaResponse:
        # DOMAIN MAPPINGS (SİZİN GENİŞ LİSTENİZ)
        DOMAIN_MAPPINGS = {
            "miras_hukuku": {
        ...
        # Domain algılama
        domain_key = context.detected_domain.lower().replace(" ", "_")
        mapping = DOMAIN_MAPPINGS.get(domain_key, DOMAIN_MAPPINGS["genel_hukuk"])

        # Sizin yazdığınız gelişmiş domain focus
        domain_focus = self.generate_domain_focus(context, DOMAIN_MAPPINGS)

        # SENIOR PROMPT (GÜNCELLENDİ)
        prompt = f"""
        ROL: {role} (SENIOR LEVEL)
        BAĞLAM: Türk Hukuku - {context.detected_domain.upper()}
        ...
        KURALLAR:
        - Cevabında yukarıdaki madde numaralarını ve içtihat ilkelerini açıkça zikret.
        - Genel hukuk yazma, doğrudan olaya ve domain'e özel konuş.
        - Yeni kural üretme, sadece mevcut hukuku uygula.
        - Maksimum 2-3 paragraf, net ve profesyonel dil.
        - Emsal karar numarası (E./K.) yoksa uydurma, "Emsal karar bulunamadı" de.

        ⚠️ HALÜSİNASYON YASAĞI: Madde numaralarını sadece doğrulanmış metinlerden kullan. Uydurma yapma veya yorum katma.

        ÇIKTI:
        """

        try:
            # LLM çağrısı - Prompt Guard eklendi
            messages = [{"role": "system", "content": LegalConfig.PROMPT_GUARD},
                        {"role": "user", "content": prompt}]
            result = self.llm.invoke(messages).content.strip()
        except Exception as e:
            print(f"LLM hatası ({role}): {e}")
            result = f"{role}: Teknik hata nedeniyle beyan oluşturulamadı."
    ...
    # --- YENİ: ÇAPRAZ DOĞRULAMA METODU ---
    def _cross_verify_personas(self, responses: List[PersonaResponse], context: QueryContext) -> List[PersonaResponse]:
        """
        Personaların beyanlarını okur, çelişkileri bulur ve 'Bilirkişi'yi revize eder.
        """
        print("   ⚖️  Persona Çapraz Doğrulaması Yapılıyor...")

        text_dump = "\n".join([f"--- {r.role} ---\n{r.response}" for r in responses])

        audit_prompt = f"""
        SEN YÜKSEK MAHKEME BAŞDENETÇİSİSİN.
        Aşağıdaki 3 hukuki görüşü incele (Davacı, Davalı, Bilirkişi).

        GÖREVLER:
        1. Hangi görüşte "uydurma içtihat" veya "aşırı yorum" var tespit et.
        2. Bilirkişinin görüşü yeterince net mi? Değilse, daha keskin ve köşeli hale getir.
        3. Davacı veya Davalı yalan söylüyorsa veya abartıyorsa, Bilirkişi bunu düzeltsin.
        4. Sonuçta BİLİRKİŞİ için revize edilmiş, kısa ve net bir mütalaa yaz.

        GÖRÜŞLER:
        {text_dump}

        ÇIKTI (Sadece Revize Edilmiş BİLİRKİŞİ Görüşünü Yaz):
        """

        try:
            revised_expert_text = self.llm.invoke(audit_prompt).content.strip()
            # Bilirkişi cevabını güncelle
            for r in responses:
                if "BİLİRKİŞİ" in r.role:
                    r.response = f"[YÜKSEK MAHKEME DENETİMİNDEN GEÇMİŞ GÖRÜŞ]\n{revised_expert_text}"
        except Exception as e:
...
"""
    ...
    # -------------------------------------------------------------------------
        # -------------------------------------------------------------------------
        # GÜNCELLEME: SENIOR LEVEL + DOMAIN MAPPING (HALÜSİNASYON ENGELLEYİCİ)
        # -------------------------------------------------------------------------
    def generate_final_opinion(self, story, topic, context_str, context: QueryContext, judge_reflex=None):
            ...
            # 1. DOMAIN MAPPING (HALÜSİNASYON KİLİDİ)
            # Modelin kafasına göre madde uydurmasını engellemek için doğru adresleri veriyoruz.
            DOMAIN_MAPPINGS = {
                "miras_hukuku": {
            ...
            # 2. HAKİM EĞİLİM KİLİDİ (REFLEX LOCK)
            reflex_note = ""
            if judge_reflex:
                reflex_note = f"""
            ...
            # 3. SENIOR PROMPT İNŞASI
            prompt = f"""
    SEN TÜRK CUMHURİYETİ HAKİMİSİN.
    Önünde bir dosya var ve bu dosya için resmi "GEREKÇELİ KARAR" yazman gerekiyor.
    {LegalConfig.PROMPT_GUARD}

    BAĞLAM: {context.detected_domain.upper() if context else 'GENEL'}

    ZORUNLU REFERANSLAR (BU MADDELERİ KULLAN):
    - İlgili Kanun Maddeleri: {mapping['maddeler']}
    - İlgili Yargıtay Daireleri: {mapping['ictihatlar']}
    (UYARI: Gerekçeni yazarken bu madde numaralarına atıf yap.)

    OLAY VE TALEP: 
    {story}

    HUKUKİ ZEMİN (DOSYADAKİ DELİLLER):
    {context_str}

    {reflex_note}

    GÖREVİN:
    Aşağıdaki şablona uygun, HUKUKİ VE SOMUT bir gerekçe metni yaz.
    Genel geçer ifadeler kullanma. Boşluk bırakma. "[...]" koyma. Metni tamamen doldur.

    ŞABLON (BU BAŞLIKLARI KULLAN):

    1. HUKUKİ NİTELEME
       (Davanın türü nedir? Yukarıdaki ZORUNLU REFERANSLAR bölümündeki madde numaralarını kullanarak hukuki tavsif yap.)

    2. DELİLLERİN VE EMSALLERİN DEĞERLENDİRİLMESİ
       (Dosyada sunulan emsal kararların [Emsal Karar] olaya uyup uymadığı, Yargıtay'ın yerleşik içtihadının ne yönde olduğu tartışılacak. Varsa Esas/Karar numaraları metin içinden alınıp yazılacak.)

    3. VİCDANİ KANAAT VE GEREKÇE
       (Neden bu karara vardın? Hangi hukuki ilke seni bu sonuca götürdü? Hakim refleksindeki tereddütleri gerekçeye yansıt.)

    4. HÜKÜM SONUCU
       (Dava KABUL / RED / KISMEN KABUL. Hüküm fıkrasını net bir dille kur.)

    KURALLAR:
    - Asla "gerekçe yazılacaktır" deme, YAZ.
    - Yargıtay kararı varsa "Esas Numarasını" metin içinden bulup mutlaka yaz.
    - Üslubun tam bir hakim gibi resmi, emredici ve "Türk Milleti Adına" karar verir ciddiyette olsun.
    """

            try:
                full_res = self.llm.invoke(prompt).content.strip()
                return self.sanitizer.enforce_no_repeat(full_res)
            except Exception as e:
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                action_engine = ActionEngine(self.risk_llm)  # Reuse LLM
                ...
                full_advice = self.judge.generate_final_opinion(
                    ...
                    context=ctx,  # Zorunlu parametre
                    judge_reflex=reflex  # Opsiyonel
                )
                ...
                report_orchestrator = ReportOrchestrator(
                    reporters=[
                        ClientSummaryPDF(),  # pdf_reports.py içindeki basit legacy
