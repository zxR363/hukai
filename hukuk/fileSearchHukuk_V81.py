...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V81: RISK LOGGER)
# ==================================================
class LegalMemoryManager:
    def __init__(self, client, embedder, llm):
        ...
        self.domain_cache = {}
        self.last_recalled_query = None  # V81: Log tekrarını önlemek için
...
"""
    ...
    def _calculate_case_success_probability(self, principle_confidence, trend_direction, conflict, domain_match):
        ...
        if trend_direction == "up":
            score += 10
            reasons.append("İçtihat eğilimi güçleniyor (+10)")
        elif trend_direction == "down":
            score -= 10
            reasons.append("İçtihat eğilimi zayıflıyor (-10)")

        if conflict:
            score -= 15
            reasons.append("Çelişkili içtihatlar mevcut (-15)")

        if not domain_match:
            score -= 10
            reasons.append("Hukuk alanı tam örtüşmüyor (-10)")

        score = max(0, min(100, round(score, 1)))
        confidence_level = "Yüksek" if score >= 70 else "Orta" if score >= 40 else "Düşük"
        if not reasons: reasons.append("Nötr faktörler")
    ...
    def recall_principles(self, query_text):
        try:
            ...
            # --- V81: RİSK LOGLAMA & RAPORLAMA ---
            memory_text = f"\n💡 YERLEŞİK İÇTİHAT HAFIZASI ({query_domain} Alanı):\n"

            for item in sorted_hits:
                risk_data = self._calculate_case_success_probability(
                    item["conf"],
                    item["trend_dir"],
                    item["conflict"],
                    item["domain_match"]
                ...
                # --- V81 EKLENTİSİ: KONSOL LOGU (Sadece sorgu değiştiyse bas) ---
                if self.last_recalled_query != query_text:
                    print("\n" + "-" * 50)
                    print(f"📊 [RİSK & BAŞARI ANALİZİ] (V81)")
                    print(f"   🎯 Sorgu Alanı: {query_domain}")
                    print(
                        f"   🎲 Başarı İhtimali: %{risk_data['success_probability']} ({risk_data['confidence_level']})")
                    print(f"   📈 Trend Yönü: {item['trend_dir']}")
                    print(f"   ⚠️ Çelişki: {'Var' if item['conflict'] else 'Yok'}")
                    print(f"   📝 Gerekçeler:")
                    for reason in risk_data['explanation']:
                        print(f"      - {reason}")
                    print("-" * 50 + "\n")

                warning = "⚠️ [YARGISAL ÇELİŞKİ]" if item["conflict"] else ""
                memory_text += f"- {warning} [{item['domain']}] {item['text']}\n"
                memory_text += f"  🎲 BAŞARI İHTİMALİ: %{risk_data['success_probability']} ({risk_data['confidence_level']})\n"
                memory_text += f"  📝 RİSK ANALİZİ: {', '.join(risk_data['explanation'])}\n"

            self.last_recalled_query = query_text  # Durumu güncelle
            return memory_text
        except:
            return ""
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V81)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v81",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V81: Risk Logger)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V81: Risk Logger):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V81: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V81.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V81 (Risk Logger & Visibility)...")
