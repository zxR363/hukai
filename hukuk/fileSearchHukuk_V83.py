...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V83: JUDGE TONE LAYER)
# ==================================================
...
"""
    ...
    # --- V83 YENİ: HAKİM DİLİ DÖNÜŞTÜRÜCÜSÜ ---
    def _generate_judicial_reasoning(self, explanation_payload):
        """
        Sayısal veriyi alır, Türk Hakimi üslubuyla gerekçeli metne çevirir.
        """
        prompt = f"""
SEN TÜRK HAKİMİSİN.
...
"""
    ...
    def recall_principles(self, query_text):
        try:
            ...
            for item in sorted_hits:
                ...
                # V83: Hakim Dili Dönüşümü
                judicial_text = self._generate_judicial_reasoning(analysis)

                if self.last_recalled_query != query_text:
                    print("\n" + "-" * 60)
                    print(f"📊 [RİSK & BAŞARI ANALİZİ] (V83: Judge's Voice)")
                    print(f"   🎯 Sorgu Alanı: {query_domain}")
                    print(f"   🎲 Başarı İhtimali: %{analysis['success_probability']} ({analysis['confidence_level']})")
                    print(f"   ⚖️ YARGISAL GEREKÇE: {judicial_text}")  # Loga da ekledim
                    print(f"   ✅ GÜÇLÜ YÖNLER:")
                    for kf in analysis['key_factors']: print(f"      + {kf}")
                    print(f"   ⚠️ RİSK FAKTÖRLERİ:")
                    if analysis['risk_factors']:
                        for rf in analysis['risk_factors']: print(f"      - {rf}")
                    else:
                        print(f"      - Belirgin risk yok.")
                    print("-" * 60 + "\n")

                warning = "⚠️ [YARGISAL ÇELİŞKİ]" if item["conflict"] else ""
                memory_text += f"- {warning} [{item['domain']}] {item['text']}\n"
                # Rapor metninde artık sadece kuru sayı değil, Hakim Gerekçesi var
                memory_text += f"  ⚖️ DEĞERLENDİRME: {judicial_text}\n"
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V83)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v83",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V83: Judge's Voice)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V83: Judge's Tone):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V83: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V83.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V83 (Judge's Tone Layer)...")
