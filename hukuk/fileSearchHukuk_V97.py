...
class ActionableRecommendationEngine:
        ...
        for concern in judge_concerns:
            ...
            recommendations.append({
                ...
                "evidence": {
                    ...
                    "source": self._infer_source(concern),  # V97 Yeni Helper
                    "count": self._estimate_count(category)  # V97 Yeni Helper
                },
                ...
                "if_not_done": self._generate_risk_note(concern),  # V97 Yeni Helper
                "why": concern
            })
    ...
    def _category_to_turkish(self, category):
        if category == "DELIL": return "delil ve ispat"
        if category == "ICTIHAT": return "emsal içtihat"
        if category == "USUL": return "usul hukuku"
        if category == "TALEP_DARALTMA": return "stratejik talep"
        return "hukuki"
...
"""
    ...
    def _calculate_case_success_probability(self, principle_confidence, trend_direction, conflict, domain_match):
        ...
        if score >= 70:
            conf_level = "Yüksek"
            summary = "Başarı ihtimali yüksek."
        elif score >= 40:
            conf_level = "Orta"
            summary = "Başarı ihtimali orta, riskli."
        else:
            conf_level = "Düşük"
            summary = "Başarı ihtimali düşük."
    ...
    def _simulate_net_decision(self, personas):
        ...
        for name, data in personas.items():
            d_val = dir_map.get(data["direction"], 0)
            s_val = stance_map.get(data["stance"], 0.6)
            c_val = conf_map.get(data["confidence_level"], 0.7)
            w_val = weights.get(name, 0)

            score = d_val * s_val * c_val * w_val
        ...
        try:
            ...
            for item in sorted_hits:
                ...
                if self.last_recalled_query != query_text:
                    print("\n" + "=" * 70)
                    print(f"📊 [OPERASYONEL STRATEJİ VE İŞ PAKETİ] (V97: Operational)")
                    print(f"   🎯 KONU: {query_text} | ⚖️ DURUM: {net_decision['decision']}")
                    print("-" * 70)
                    print(f"📝 YÖNETİCİ ÖZETİ: \"{exec_summary[:120]}...\"")
                    print("-" * 70)

                    # V97 YENİ LOG: YAPILANDIRILMIŞ İŞ PAKETLERİ
                    ...
                    for act in action_plan:
                        ...
                        print(f"      🔎 Kaynak: {act['evidence']['source']} (Adet: {act['evidence']['count']})")
                        print(
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V97)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v97",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V97: Operational)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V97: Operational Strategy):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V97: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V97.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V97 (Operational Strategy: Work Orders)...")
