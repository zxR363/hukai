...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V86: DEVIL'S ADVOCATE / REJECTION SIMULATOR)
# ==================================================
...
"""
    ...
    def _generate_judicial_reasoning(self, explanation_payload):
        prompt = f"""
SEN TÜRK HAKİMİSİN.
GÖREVİN: Aşağıdaki değerlendirme verilerini, yargısal gerekçe diline uygun şekilde yeniden yazmak.

KURALLAR:
- Tarafsız ve ölçülü ol
- Kesin hüküm kurma
- "Kanaat", "Takdir", "Dosya kapsamı", "Yerleşik uygulama" gibi terimler kullan
- Akademik ve resmi üslup
...
"""
        try:
            ...
            return "Hukuki değerlendirme oluşturulamadı."
...
GÖREVİN:
Verilen verileri savcılık değerlendirme diliyle yeniden yaz.

KURALLAR:
- Şüphe ve kamu düzeni vurgusu
- Kesin hüküm yok
- “kuvvetli şüphe”, “somut emare”, “kamu düzeni” kavramlarını kullan
- Resmi ve akademik üslup
- Kısa bir paragraf olsun

VERİLER:{json.dumps(explanation_payload, ensure_ascii=False)}

ÇIKTI:
Yalnızca savcılık değerlendirme metni üret.
"""
        try:
            return self.llm.invoke(prompt).content.strip()
        except:
            return "Savcılık değerlendirmesi oluşturulamadı."

    # --- V85: BİLİRKİŞİ DİLİ ---
    def _generate_expert_witness_reasoning(self, explanation_payload):
        prompt = f"""
SEN TARAFSIZ BİR HUKUK BİLİRKİŞİSİSİN.

GÖREVİN:
Verilen verileri tarafsız, ölçülebilir ve teknik bilirkişi raporu diliyle yeniden yaz.

KURALLAR:
- Yorumdan kaçın, sadece tespit yap
- "Tespit edilmiştir", "Veriler incelendiğinde", "Oransal olarak" gibi kalıplar kullan
- Sayısal ve eğilim odaklı yaz (Örn: Eğilim güçlenmektedir)
- Akademik ve teknik üslup
- Kısa bir paragraf olsun

VERİLER:{json.dumps(explanation_payload, ensure_ascii=False)}

ÇIKTI:
Yalnızca bilirkişi raporu metni üret.
"""
        try:
            return self.llm.invoke(prompt).content.strip()
        except:
            return "Bilirkişi tespiti yapılamadı."

    # --- V86 YENİ: RED GEREKÇESİ SİMÜLATÖRÜ (DEVIL'S ADVOCATE) ---
    def _generate_rejection_reasoning(self, explanation_payload):
        prompt = f"""
SEN TÜRK HAKİMİSİN.

GÖREVİN:
Aşağıdaki değerlendirme verilerine dayanarak, davayı REDDEDECEK olsaydın,
hangi hukuki gerekçelerle reddedeceğini yaz. (Şeytanın Avukatı Modu)

KURALLAR:
- Davacı aleyhine yorumla
- Verilerdeki çelişkileri ve zayıf noktaları (risk faktörlerini) öne çıkar
- Kesinlik iddiası yok, gerekçeli ret dili kullan
- Yargısal gerekçe dili (Örn: "Her ne kadar... ise de...", "kanaatine varılmıştır")
- Kısa bir paragraf olsun

VERİLER:{json.dumps(explanation_payload, ensure_ascii=False)}

ÇIKTI:
Yalnızca RED gerekçesi metni üret.
"""
        try:
            return self.llm.invoke(prompt).content.strip()
        except:
            return "Red gerekçesi oluşturulamadı."
    ...
    def recall_principles(self, query_text):
        try:
            ...
            for item in sorted_hits:
                ...
                # PERSONA ÜRETİMİ (V86: DÖRTLÜ YAPI)
                ...
                rejection_text = self._generate_rejection_reasoning(analysis)  # V86 Eklentisi

                if self.last_recalled_query != query_text:
                    print("\n" + "-" * 60)
                    print(f"📊 [RİSK & BAŞARI ANALİZİ] (V86: The Full Bench)")
                    print(f"   🎯 Sorgu Alanı: {query_domain}")
                    print(f"   🎲 Başarı İhtimali: %{analysis['success_probability']} ({analysis['confidence_level']})")
                    print(f"   ⚖️ HAKİM: {judicial_text[:100]}...")
                    print(f"   🏛️ SAVCI: {prosecutor_text[:100]}...")
                    print(f"   📈 BİLİRKİŞİ: {expert_text[:100]}...")
                    print(f"   🛑 RED GEREKÇESİ: {rejection_text[:100]}...")  # Loga eklendi
                    print("-" * 60 + "\n")
                ...
                memory_text += f"  📈 BİLİRKİŞİ: {expert_text}\n"
                memory_text += f"  🛑 MUHTEMEL RED GEREKÇESİ: {rejection_text}\n"  # Rapora eklendi
                memory_text += f"  (Teknik Skor: %{analysis['success_probability']})\n"
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V86)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v86",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V86: Full Bench)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V86: The Full Bench):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V86: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V86.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V86 (Devil's Advocate: Rejection Simulator)...")
