...
class StrengtheningAction:
    ...
    related_doubt: str
    impact_score: int
    # [V120 GÜNCELLEMESİ] Kaynak ve Risk alanları eklendi
    source_ref: str = "Genel Hukuk İlkeleri"
    risk_analysis: str = "Hak kaybı riski mevcuttur."
...
class PersonaEngine:
    ...
    def build_persona_prompts(self, context: QueryContext, decision_context: DecisionContext,
        ...
        # [GÜNCELLEME - PDF İSMİ DÜZELTME]
        # Avukatın isteği: "TürkMedeniKanunu.pdf" gibi teknik isimler yerine hukuki tanım kullanılsın.
        formatted_docs = []
        for d in decision_context.documents[:3]:
            source_name = d['source']
            if "TürkMedeniKanunu" in source_name or "TMK" in source_name:
                source_name = "Yasal Mevzuat Hükümleri ve Yargıtay İçtihatları"
            elif ".pdf" in source_name:
                source_name = source_name.replace(".pdf", "").replace("_", " ").title()

            formatted_docs.append(f"- {source_name} ({d['role']}): {d['reason']}")

        # [YENİ] AVUKATIN İSTEDİĞİ USUL HUKUKU BİLGİ SETİ
        # Bu metin, AI'ın hafızasına "Usul Kuralları" olarak kazınacak.
        usul_bilgi_notu = """
                === ZORUNLU USUL HUKUKU KONTROL LİSTESİ (HMK) ===
                Rapora konu olayda aşağıdaki hususları MUTLAKA değerlendir:
                1. GÖREV KURALLARI (HMK m.1-2): Görev kuralları kamu düzenindendir. Mahkemenin görevli olup olmadığını kontrol et.
                2. DAVA ŞARTI (HMK m.114/1-c): Görev bir dava şartıdır.
                3. RE'SEN İNCELEME (HMK m.115): Mahkeme dava şartlarını (görev dahil) yargılamanın her aşamasında kendiliğinden (re'sen) inceler.
                """
        ...
        # 3. Rolleri Tanımla (GÜNCELLENMİŞ VERSİYON)
        # Buradaki talimatları "Özetle" yerine "Beyanda Bulun" şeklinde değiştiriyoruz.
        roles_config = [
            ("DAVACI VEKİLİ",
             "Müvekkil lehine, dosya kapsamındaki delillere ve Yargıtay içtihatlarına dayanarak ESASA İLİŞKİN BEYANDA bulun. Hakimi davanın kabulüne ikna et."),
            ("DAVALI VEKİLİ",
             """Müvekkil lehine, Davacı iddialarını ÇÜRÜT.

             ⚠️ GÖREVİN: DOSYADAKİ "HUKUKİ GERÇEĞİ" GÖRMEZDEN GELEREK, MÜVEKKİLİNİ KURTARACAK BİR "USULİ BAHANE" VEYA "ESAS İTİRAZI" ÜRETMEKTİR.

             ŞU ALGORİTMAYI UYGULA (ADIM ADIM):

             1. BAĞLAM ANALİZİ: Dosyanın türünü tespit et (Miras mı, İş mi, Ticaret mi, Ceza mı?).

             2. STRATEJİ SEÇİMİ (Dosya Türüne Göre Birini Seç ve Uygula):

                A) EĞER MİRAS/VERASET DAVASI İSE:
                   - Savunma: "Bu dava çekişmesiz yargı işi değildir, taraflar arasında husumet vardır/muhtemeldir."
                   - Talep: "Sulh Hukuk değil, ASLİYE HUKUK MAHKEMESİ görevlidir. Görevsizlik kararı verilmelidir."

                B) EĞER İŞ/ALACAK/TAZMİNAT DAVASI İSE:
                   - Savunma: "Talep zamanaşımına uğramıştır (TBK/İşK)" VEYA "Davacı iddiasını somut delillerle ispatlayamamıştır (HMK m.190)."
                   - Talep: "Davanın zamanaşımı ve ispat yokluğu nedeniyle reddi gerekir."

                C) EĞER CEZA DOSYASI İSE:
                   - Savunma: "Elde edilen deliller hukuka aykırıdır (Zehirli ağacın meyvesi)" VEYA "Şüpheden sanık yararlanır ilkesi gözetilmemiştir."
                   - Talep: "Müvekkilin beraatine karar verilmelidir."

                D) DİĞER/GENEL DURUMLAR İSE:
                   - Savunma: "Dava şartları oluşmamıştır (HMK 114)" VEYA "Mahkemeniz yetkisizdir (Yer yönünden)."

             3. ÇIKTI KURALI (KESİN YASAKLAR):
                ❌ ASLA "Dosyadaki belgelerden anlaşıldığı üzere mahkemeniz görevlidir" DEME.
                ❌ ASLA "Davacı haklıdır" anlamına gelecek bir cümle kurma.
                ✅ Dosyadaki belge/emsal senin aleyhine olsa bile (Örn: 'Sulh Hukuk Görevlidir' dese bile), sen yukarıdaki stratejiyi uygulayarak "HAYIR, GÖREVSİZDİR" diyeceksin.

             AMACIN: Hakimin kafasını karıştırmak, süreci uzatmak ve dosyayı reddettirmek.
             """),
            ("BİLİRKİŞİ",
             "YÜKSEK MAHKEME TETKİK HAKİMİ gibi davran. Tarafsız, teknik ve hukuki bir dille dosyadaki eksiklikleri ve uyumsuzlukları 'Tespit Edilmiştir' diliyle raporla.")
        ]
        ...
        for role, instruction in roles_config:
            ...
            target_party = "Davalı tarafın" if "DAVACI" in role else "Davacı tarafın"

            final_prompt = f"""
            GÖREV: Aşağıdaki HUKUKİ DOSYAYI, kıdemli bir {role} olarak değerlendir.Mahkeme huzurunda, hakime hitaben SON BEYANLARINI (Esasa İlişkin Beyan) sunuyorsun.

            ⚠️ [YASAKLI İFADELER - BUNLARI ASLA KULLANMA]:
            ❌ "İtiraz ediyorum" (Bu Amerikan filmlerinde olur, Türk hukukunda olmaz)
            ❌ "Müvekkil lehine itiraz ederim"
            ❌ "Kabul etmiyoruz" (Çok basit kalır)
            ❌ "Özetle şöyledir"
            ❌ "Davacı dilekçesinde..." (3. şahıs anlatımı yasak)

            ✅ [ZORUNLU HUKUKİ KALIPLAR - BUNLARI KULLAN]:
            1. RED VE İNKAR İÇİN ŞUNLARI SEÇ:
               - "{target_party} beyanlarının KABULÜ MÜMKÜN DEĞİLDİR."
               - "{target_party} iddialarının BU HALİYLE KABULÜ MÜMKÜN DEĞİLDİR."
               - "{target_party} beyanlarının ve taleplerinin TÜMÜYLE KABULÜ MÜMKÜN DEĞİLDİR."
               - "Hukuki dayanaktan yoksun iddiaların reddi gerekmektedir."

            2. USULİ İTİRAZLAR İÇİN (Özellikle Davalı isen):
               - "HMK m.114/1-c gereği görev dava şartıdır."
               - "Mahkemenizce HMK m.115 uyarınca re'sen gözetilmelidir."

            3. OLUMLAMA VE TALEP İÇİN:
               - "Dosya münderecatından anlaşılacağı üzere..."
               - "Müvekkilin haklılığı Yargıtay içtihatları ile sabittir."
               - "Yukarıda açıklanan nedenlerle..."
               - "Arz ve talep ederiz."


            ⚠️ KESİNLİKLE YAPMA:
            - "Davacı dilekçesinde şöyle demiştir" gibi 3. tekil şahıs anlatımı yapma.
            - Hikaye anlatır gibi özetleme.

            ✅ MUTLAKA YAP (FORMAT VE ÜSLUP):
            1. DOĞRUDAN KONUŞ: "Müvekkilimiz...", "Huzurdaki davada...", "Kanaatimizce..." ifadelerini kullan.
            2. HUKUKİ KALIPLAR KULLAN: Cümlelerini "Arz ve talep ederiz", "Hukuki dayanaktan yoksundur", "Dosya münderecatından anlaşılacağı üzere" gibi profesyonel kalıplarla kur.
            3. ATIF YAP: Aşağıdaki "MEVCUT BELGELER" kısmındaki delillere dayanarak konuş.
            4. SONUÇ BAĞLA: Beyanını mutlaka net bir talep cümlesiyle bitir (Örn: "Davanın kabulüne karar verilmesini vekileten arz ve talep ederiz.")
            ...
            HAKİMİN TEREDDÜTLERİ (Bunları gidermeye çalış):
            ...
            ÇIKTI KURALLARI:
            1. Doğrudan "Sayın Hakim," veya "Mahkemenizden talebimiz şudur ki;" diyerek başla.
            2. Sadece "MEVCUT BELGELER" kısmındaki gerçek delillere dayan. Olmayan kanun uydurma.
            3. Maksimum 4-5 cümle. Net, keskin ve profesyonel ol.
            4. Beyanını mutlaka net bir talep cümlesiyle bitir (Örn: "...karar verilmesini vekileten arz ve talep ederiz.")

            ÇIKTI (Sadece Beyan Metni):
            """
            ...
            try:
                ...
                try:
                    ...
                    if "429" in err_str or "Rate limit" in err_str:
                        ...
                        fallback_llm = ChatOllama(model="qwen2.5:7b", temperature=0.3,
                                                  base_url="http://192.168.134.42:11434")
...
class AppealPetitionGenerator:
    """
    [GÜNCELLEME] Avukat talebiyle 5 aşamalı profesyonel dilekçe formatı:
    1. Dava Safahatı
    2. Karar Özeti
    3. Usul İtirazları
    4. Esas İtirazları
    ...
    def generate(self, judge_reasoning, case_topic):
        prompt = f"""
        GÖREV: SEN, YÜKSEK YARGI TECRÜBESİ OLAN KIDEMLİ BİR AVUKATSIN.
        Aşağıdaki yerel mahkeme gerekçesine karşı, Bölge Adliye Mahkemesi'ne (İstinaf) sunulmak üzere PROFESYONEL BİR İSTİNAF DİLEKÇESİ hazırla.

        {LegalConfig.PROMPT_GUARD}

        DİLEKÇE İÇERİĞİ KESİNLİKLE ŞU 5 BAŞLIKTAN OLUŞMALIDIR (Sırayı Bozma):

        I. YEREL MAHKEMEDE GÖRÜLEN DAVA SAFAHATI
           - Davanın nasıl açıldığı, tarafların temel iddiaları ve yargılama sürecinin kısa ve özlü bir hikayesi (Distografisi).
           - (Bilmediğin tarihler için [TARİH] yaz, uydurma).

        II. YEREL MAHKEME KARARININ ÖZETİ
           - Mahkemenin aleyhe kurduğu hüküm ve dayandığı gerekçenin özeti.

        III. USULE İLİŞKİN İTİRAZLARIMIZ (HMK Kapsamında)
           - Eksik inceleme, delillerin toplanmaması, hukuki dinlenilme hakkı ihlali, görev/yetki itirazları.
           - Mutlaka HMK maddelerine atıf yap.

        IV. ESASA İLİŞKİN İTİRAZLARIMIZ (Maddi Hukuk Kapsamında)
           - Kanunun yanlış uygulanması, delillerin hatalı değerlendirilmesi, Yargıtay içtihatlarına aykırılık.
           - Somut olaydaki haksızlığı vurgula.

        V. SONUÇ VE İSTEM
           - "Yukarıda arz ve izah edilen nedenlerle..." diye başlayan, kararın kaldırılmasını ve davanın lehe sonuçlanmasını isteyen net kapanış.

        BAĞLAM VERİLERİ:
        DOSYA KONUSU: {case_topic}
        YEREL MAHKEME GEREKÇESİ: {judge_reasoning}

        BİÇİM KURALLARI:
        - En üste standart dilekçe başlığı at: "[İL] BÖLGE ADLİYE MAHKEMESİ İLGİLİ HUKUK DAİRESİNE Sunulmak Üzere [YEREL MAHKEME ADI] HAKİMLİĞİNE"
        - Dosya No kısmını boş bırak: "Dosya No: .../..."
        - Dil son derece resmi, saygılı ama itirazlarda kararlı olsun.
        - Asla "Ben bir yapay zekayım" deme.

        ÇIKTI (Sadece Dilekçe Metni):
        """

        print(f"   ⏳ LLM DÜŞÜNÜYOR: [İSTİNAF DİLEKÇESİ - 5 Aşamalı] Yazılıyor...")
        try:
            # self.llm invoke kullanımı (LegalApp içindeki yapıya uygun)
            res = LegalUtils.safe_extract_content(self.llm.invoke(prompt))
            print(f"   ✅ LLM YANITLADI: [İSTİNAF DİLEKÇESİ]")
            return res
        except Exception as e:
            print(f"Dilekçe hatası: {e}")
            return "Dilekçe teknik bir hata nedeniyle oluşturulamadı."
...
class LegalSearchEngine:
    ...
    def retrieve_raw_candidates(self, full_query):
        ...
        #     # [DÜZELTME] self.judge yerine global fonksiyonu kullanıyoruz
        #     print(f"      🤖 MCP için LLM hazırlanıyor...")
        #     temp_llm_for_search = get_llm_by_profile("judge")
        #     search_keywords = temp_llm_for_search.invoke(keyword_prompt).content.strip()
        #
...
------------------------------------------------------------
Burada:
1. Öncelikle arama sonuçlarında gelen [MEVZUAT] etiketli belgeleri temel al.
2. Somut olayla doğrudan bağlantıyı belirt.
3. Yorum yapma, normu açıkla.
4. Aynı kanun maddesini birden fazla kez özetleme.
5. Her madde numarasını sadece bir kez belirt.
6. EĞER arama sonuçlarında eksiklik varsa; yukarıda sana verilen "ZORUNLU HUKUKİ REFERANSLAR" listesindeki maddeleri (Örn: TMK 598, HMK 114) kontrol et.
7. Bu maddelerden SADECE somut olayla ve dava türüyle DOĞRUDAN İLGİLİ olanları, kendi hukuki bilginden (knowledge base) tamamlayarak listeye ekle.
8. Olayla ilgisi olmayan maddeleri listeye alma.
9. İlgili kanun maddelerini KISA ve NET şekilde özetle.
...
------------------------------------------------------------
C. SONUÇ VE HUKUKİ TAVSİYE (BU BÖLÜMÜ BOŞ BIRAKMA, DETAYLI YAZ)
------------------------------------------------------------
Aşağıdaki her bir başlığın altına, bir avukatın müvekkiline vereceği net tavsiyeleri 1-2 paragraf halinde yaz:

1. DOSYANIN ZAYIF YÖNLERİ NELERDİR?
    (Buraya dosyanın eksiklerini ve risklerini yaz)

2. GÜÇLENDİRİLMESİ GEREKEN NOKTALAR:
    (Buraya hangi delillerin veya beyanların eksik olduğunu yaz)

3. NET BİR YOL HARİTASI:
    (Adım adım yapılması gerekenleri maddeler halinde sırala. Örn: 1. Bilirkişi iste, 2. Tanık dinlet...)

4. DOSYANIN KABUL EDİLME İHTİMALİ:
    (Yüzde vermeden, 'Yüksek/Düşük' şeklinde gerekçesiyle yaz)

5. RED RİSKİNİ AZALTMAK İÇİN 2-3 SOMUT AKSİYON (LİSTE HALİNDE YAZ):
    Lütfen aşağıdaki formatı aynen kullanarak en az 2 somut öneri yaz:
    - [AKSİYON 1]: (Örneğin: "Tanıkların yeniden dinlenmesi için talepte bulunun çünkü...")
    - [AKSİYON 2]: (Örneğin: "Bilirkişi raporuna şu gerekçeyle itiraz edin...")
    - [AKSİYON 3]: (Varsa ek öneri)
...
class ActionEngine:
    ...
    def build_risk_payload(self, reflex, persona_outputs) -> List[Dict]:
        ...
        for doubt in doubts:
            ...
            # [GÜNCELLEME] Bilirkişi/Teknik Konular İçin Özel Talimat
            expert_instruction = ""
            if category == "DELIL" or "bilirkişi" in doubt.lower() or "rapor" in doubt.lower() or "hesap" in doubt.lower():
                expert_instruction = """
                            ÖZEL DURUM (BİLİRKİŞİ STRATEJİSİ):
                            Eğer tereddüt teknik bir konuya (hesaplama, kusur durumu, imza incelemesi vb.) dayanıyorsa, aksiyonunu şu 3 seçenekten birine göre belirle:
                            1. HİÇ RAPOR YOKSA: "Bilirkişi Raporu Alınması Talebi" öner.
                            2. RAPOR VAR AMA EKSİKSE: "Eksik hususların giderilmesi için Bilirkişi EK RAPOR Talebi" öner.
                            3. RAPOR HATALI/ÇELİŞKİLİ İSE: "Yanlış değerlendirme nedeniyle dosyanın YENİ BİR BİLİRKİŞİYE (veya Heyete) Tevdii Talebi" öner.
                            """
    ...
    def generate_appeal_strategy(self, context_str: str, judge_tendency: str) -> str:
        """
        [GÜNCELLEME] Avukat talebi üzerine:
        - Hitap (Sayın Üst Mahkeme) kaldırıldı.
        - Sadece özet ve madde madde teknik bozma sebepleri eklendi.
        """

        # Eğer dava kazanılmışsa itiraza gerek yoktur
        if "OLUMLU" in judge_tendency.upper():
            return "Karar lehe göründüğünden şu aşamada itiraz stratejisine gerek duyulmamıştır."

        prompt = f"""
           GÖREV: Aşağıdaki dava dosyasında aleyhe çıkabilecek bir karara karşı, İstinaf/Temyiz başvurusunda kullanılabilecek "OLASI HUKUKİ BOZMA SEBEPLERİNİ" belirle.

           BAĞLAM:
           {context_str[:2500]}

           ⚠️ KESİNLİKLE YAPMA:
           ❌ "Sayın Üst Mahkeme", "Yargıtay İlgili Ceza Dairesine" gibi hitaplar KULLANMA.
           ❌ Dilekçe formatında yazma. İmza, tarih atma.
           ❌ Uzun paragraflar kurma.

           ✅ MUTLAKA YAP (FORMAT):
           1. Sadece hukuki "Hata" ve "Eksiklikleri" madde madde sırala.
           2. Her madde 1-2 cümlelik net özetler olsun.
           3. Şu başlıkları kullanabilirsin: "Usul Hataları", "Eksik İnceleme", "Hatalı Delil Takdiri".

           ÖRNEK ÇIKTI FORMATI:
           • HMK m.27 Uyarınca Hukuki Dinlenilme Hakkının İhlali: Davalı tarafın tanık listesi zamanında sunulmasına rağmen mahkemece dinlenmemiştir.
           • Eksik İnceleme: Bilirkişi raporundaki çelişkiler giderilmeden hüküm kurulmuştur.
           • Delil Takdiri Hatası: Kamera kayıtları görmezden gelinmiştir.

           ÇIKTI (Sadece Maddeler):
           """

        try:
            # Buradaki çağırma yöntemi sizin `LegalApp` yapınızdaki LLM instance'ına bağlıdır.
            # Geçici olarak main LLM'i kullanıyoruz:
            messages = [
                SystemMessage(content="Sen uzman bir İstinaf Avukatısın. Sadece teknik bozma sebeplerini listele."),
                HumanMessage(content=prompt)
            ]

            # self.llm mevcutsa onu kullan
            response = self.llm.invoke(messages)
            return response.content if hasattr(response, 'content') else str(response)

        except Exception as e:
            print(f"Appeal strategy error: {e}")
            return "İtiraz stratejisi oluşturulurken teknik bir hata oluştu."
            ...
            try:
                ...
                try:
                    ...
                    if "429" in err_str or "Rate limit" in err_str:
                        ...
                        fallback_llm = ChatOllama(model="qwen2.5:7b", temperature=0.1,
                                                  base_url="http://192.168.134.42:11434")
        ...
        try:
            while True:
                ...
                for i, d in enumerate(valid_docs):
                    ...
                    # [GÜNCELLEME 3] Dosya ismini avukatın istediği teknik jargona çevirme
                    display_source = d['source']
                    if "TürkMedeniKanunu" in display_source or "TMK" in display_source:
                        display_source = "Yasal Mevzuat Hükümleri ve Kabul Gören Yargıtay İçtihatları"
                    elif ".pdf" in display_source:
                        display_source = display_source.replace(".pdf", "").replace("_", " ").title()

                    context_str += f"""
                        BELGE # {display_source}
                        ETİKET: {doc_label}
                        TÜR: {d['type']}
                        ROL: {d['role']}
                ...
                # [GÜNCELLEME] Yeni liste formatlı stratejiyi ActionEngine üzerinden çağırıyoruz
                # context_str: Dosyadaki deliller/belgeler
                # reflex.tendency: Hakimin KABUL/RED eğilimi
                appeal_args = action_engine.generate_appeal_strategy(context_str, reflex.tendency)

                # Dilekçe üretimi aynı kalabilir
