...
# ==================================================
# 1️⃣ YENİ MODÜL: PDF SONU OKUYUCU (V44 ÖZEL)
# ==================================================
...
# ==================================================
# 2️⃣ MEVCUT ARAÇLAR (V43 İLE AYNI)
# ==================================================
...
# ==================================================
# 3️⃣ INDEXING ENGINE (V43 İLE AYNI)
# ==================================================
...
def create_pdf_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V44.pdf"):
    pdf = LegalReport();
    pdf.add_page();
...
# ==================================================
# 5️⃣ ANA MOTOR (V44 SMART STITCHING)
# ==================================================
def main():
    print("🚀 LEGAL SUITE V44 (Smart Stitching: Full Verdicts)...")
    ...
    while True:
        ...
        for hit in candidates:
            ...
            if is_ok:
                # --- V44 MÜDAHALESİ: KUYRUĞU YAKALA ---
                extra_context = ""
                # Sadece Emsal Kararsa ve dosya diskte varsa
                ...
                valid_docs.append({
                    ...
                    "page": page,
                    "type": type_desc,
                    "text": doc_text + extra_context,  # Metni zenginleştir
                    "score": hit.score,
                    "reason": reason
                })
        ...
        for d in valid_docs:
            context_str += f"""
>>> BELGE TÜRÜ: {d['type']}
KAYNAK: {d['source']} (Sayfa {d['page']})
SEÇİLME NEDENİ: {d['reason']}
İÇERİK: {d['text']}
-----------------------------------
"""
        ...
        # PROMPT (STRICT MODE - V43 İLE AYNI)
        prompt = f"""
...
1. SINIRLAR (ÇOK ÖNEMLİ):
   - SADECE ve SADECE yukarıdaki "DELİLLER" kısmında verilen metinleri kullan.
   - Asla dışarıdan kanun/karar uydurma (Halüsinasyon görme).
   - Eğer delillerde konuyla ilgili bilgi yoksa, "Verilen belgelerde bu konuda bilgi bulunmamaktadır" de.

2. FORMAT (KESİNLİKLE BU ŞABLONU UYGULA):
   Yanıtını şu 3 ana başlık altında topla:

   A. MEVZUAT DAYANAKLARI (Kanun Maddeleri)
      - Burada sadece [MEVZUAT] etiketli belgelerdeki bilgileri kullan.

   B. İLGİLİ EMSAL KARARLAR (Yargıtay ve İstinaf Kararları)
      - BURASI ÇOK ÖNEMLİ. Deliller arasında [EMSAL KARAR] etiketi taşıyan belgeler varsa, bunları MUTLAKA burada özetle.
      - Metin içinde "[OTOMATİK EKLENEN KARAR SONUCU]" başlıkları varsa, oradaki nihai kararları (Onama/Bozma) rapora işle.
      - "X dosyasında belirtildiği üzere..." diyerek atıf yap.

   C. SONUÇ VE TAVSİYE
      - Hukuki bir yol haritası çiz.

3. ATIF VE KAYNAKÇA:
   - Her cümlenin sonuna parantez içinde kaynağını ekle.
   - Örn: (Kaynak: TürkMedeniKanunu.pdf, Sayfa 12)
