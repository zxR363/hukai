...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V89: REASONED DECISION DRAFT)
# ==================================================
...
"""
    ...
    # --- V89 YENİ: GEREKÇELİ KARAR YAZICISI ---
    def _generate_final_verdict_reasoning(self, net_decision, topic, trend, principles):
        """
        Matematiksel sonucu, bir hakimin nihai gerekçeli karar metnine dönüştürür.
        """
        prompt = f"""
...
- Ölçülü
- Tereddüt barındıran
- “Mahkemenin takdir yetkisi” vurgusu içeren
- Kesinlik iddiası taşımayan
olmalıdır.
...
- Kesin hüküm dili kullanma
- Alternatif görüşlerin neden baskın görülmediğini açıkla
- Hakimin neden daha ihtiyatlı davrandığını belirt
...
"""
        try:
            ...
            return "Gerekçeli karar oluşturulamadı."
    ...
    def recall_principles(self, query_text):
        try:
            ...
            for item in sorted_hits:
                ...
                # 4. Nihai Karar Simülasyonu
                net_decision = self._simulate_net_decision(persona_signals)

                # 5. V89: GEREKÇELİ KARAR TASLAĞI OLUŞTURMA
                ...
                if self.last_recalled_query != query_text:
                    print("\n" + "=" * 70)
                    print(f"📊 [RİSK, PERSONA VE KARAR SİMÜLASYONU] (V89: Verdict Draft)")
                    ...
                    print("-" * 70)
                    print(f"⚖️ NİHAİ EĞİLİM: {net_decision['decision']} (Skor: {net_decision['final_score']})")
                    print(f"✍️ GEREKÇELİ KARAR TASLAĞI:\n   {verdict_text[:200]}...\n")
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V89)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v89",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V89: Virtual Clerk)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V89: Judge's Draft):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V89: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V89.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V89 (The Virtual Clerk: Reasoned Decision Draft)...")
