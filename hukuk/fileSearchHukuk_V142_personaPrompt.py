...
class PersonaEngine:
    ...
    def build_persona_prompts(self, context: QueryContext, decision_context: DecisionContext,
        ...
        for role, instruction in roles_config:
            # --- PROMPT ŞABLONU (Burada oluşturuluyor) ---
            final_prompt = f"""
            GÖREV: Aşağıdaki HUKUKİ DOSYAYI, belirtilen PERSONA KURALLARINA göre değerlendir.
            ...
            KISITLAMALAR:
            1. Sadece yukarıda verilen "DOĞRULANMIŞ METİNLER" ve "BELGELER" üzerinden konuş.
            2. Asla olmayan bir kanun maddesi uydurma.
            3. Maksimum 3-4 cümle. Net ve hukuki terimlerle konuş.
...
class ActionEngine:
    """
    Hakim tereddütlerini azaltmaya yönelik somut hukuki aksiyonlar üretir.
    CPU (Hazırlık) ve GPU (Çalıştırma) aşamaları ayrıştırılmıştır.
    """
    ...
    def build_risk_payload(self, judge_reflex: JudgeReflex, persona_outputs: List[PersonaResponse]):
        """
        Risk promptunu ve hedef veriyi hazırlar.
        Bu fonksiyon sadece string ve mantık işlemleri yapar.
        """
        if not judge_reflex.doubts:
            return None

        # 1. Bilirkişi (Expert) Görüşünü Çek
        expert_opinion = "Bilirkişi görüşü mevcut değil."
        for p in persona_outputs:
            if "BİLİRKİŞİ" in p.role:
                expert_opinion = p.response
                break

        # 2. Stratejik Veri Paketi Hazırla
        strategy_context = {
            "target_doubt": judge_reflex.doubts[0],  # En büyük şüphe
            "current_score": judge_reflex.score,
            "expert_diagnosis": expert_opinion[:500]  # Çok uzatmadan özünü al
        }

        # 3. Prompt Hazırla (ORİJİNAL YAPI KORUNDU)
        prompt = f"""
        GÖREV: Aşağıdaki HUKUKİ SORUNU çözecek TEK ve SOMUT bir usul işlemi (aksiyon) belirle.

        === ANALİZ VERİLERİ ===
        HAKİMİN ŞÜPHESİ: {strategy_context['target_doubt']}
        BİLİRKİŞİ TESPİTİ: {strategy_context['expert_diagnosis']}

        TALİMAT:
        Genel tavsiye verme. Bilirkişinin işaret ettiği eksikliği giderecek spesifik dilekçe, delil veya talep ismini yaz.
        Sadece JSON benzeri formatta cevap ver.

        FORMAT:
        Başlık: [Kısa ve Teknik Aksiyon Adı - Örn: Islah Dilekçesi]
        Açıklama: [Hangi maddeye veya delile dayanarak ne talep edilmeli?]
        Etki: [1-10 arası tahmini katkı puanı]
        """

        print("   ✅ CPU: Risk/Aksiyon payload'ı hazırlandı.")

        return {
            "prompt": prompt,
            "target_doubt": strategy_context["target_doubt"]
        }

    # 🔴 ADIM 2: GPU ÇALIŞTIRMA (SADECE BURASI LLM KULLANIR)
        # 🔴 ADIM 2: GPU ÇALIŞTIRMA (SAF SERİ)
    def execute_action(self, payload) -> List[StrengtheningAction]:
            if not payload: return []

            print("   🛠️ GPU: Aksiyon analizi işleniyor (Serial)...")
            try:
                # Bloklayıcı çağrı
                result = self.llm.invoke(payload["prompt"]).content.strip()
                return [self._parse_action(result, payload["target_doubt"])]
            except Exception as e:
    ...
    def _parse_action(self, text: str, doubt: str) -> StrengtheningAction:
        ...
        desc = "Eksikliğin giderilmesi için beyan sunulmalıdır."
        impact = 5
...
GÖREV:
Bu belgenin olayla hukuki alakasını ve rolünü belirle.

KURALLAR:
1. Eğer belge tamamen alakasızsa veya yasaklı kapsamdaysa sadece "KARAR: RED" yaz.
2. Eğer belge alakalıysa:
   - "KARAR: KABUL" yaz.
   - Rolünü belirle: [DOĞRUDAN DELİL] (Olayla birebir) veya [EMSAL İLKE] (Benzer hukuk).
   - Tek cümlelik gerekçe yaz.

ÇIKTI FORMATI (SADECE BUNU YAZ):
Örnek 1: KARAR: RED (Gerekçe: Konu farklı)
Örnek 2: KARAR: KABUL | ROL: [EMSAL İLKE] | GEREKÇE: İspat yükü kuralını açıklar.
"""
            payloads.append({
                "prompt": prompt,
                "source": source,
