...
GÖREVİN:
Bu ilke ışığında, olayı değerlendirirken yaşadğin EN FAZLA 3 TEMEL TEREDDÜTÜ (Doubts) listele.
...
class LegalReporter:
    ...
    def create_report(user_story, valid_docs, advice_text, audit_data=None, filename="Hukuki_Rapor_V120.pdf", llm=None,
        ...
        if audit_data:
            ...
            if llm:
                # EK-1: HAKİM KARAR GEREKÇESİ
                reasoning_gen = JudgeReasoningGenerator(llm)
                # V125 GÜNCELLEME: Tüm bağlamı geçiriyoruz
                judge_text = reasoning_gen.generate(
                    audit_logs=audit_data,
                    story=user_story,
                    context_str=advice_text  # Tüm belge özeti ve tavsiye metni
                )
