...
"""
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V66: Learning Mode)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V66.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V66 (Learning Mode: Memory & Principles)...")
        ...
        try:
            while True:
                ...
                for i, d in enumerate(valid_docs):
                    # Yargıcın gerekçesini (reason) öne çıkartıyoruz, metni (text) kısaltıyoruz
                    context_str += f"""
                        BELGE #{i + 1}
                        KAYNAK: {d['source']}
                        TÜR: {d['type']}
                        ROL: {d['role']}
                        YARGIÇ GEREKÇESİ (ÖNEMLİ): {d['reason']}
