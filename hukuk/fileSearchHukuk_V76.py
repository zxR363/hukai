...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V76: DETAILED ANALYTICS)
# ==================================================
...
"""
    ...
    # --- V76 YENİ: DOSYA İSİMLERİ İLE LOGLAMA ---
    def _save_principle_v76(self, text, confidence, source_ids, domain, cluster_data):
        try:
            vec = self.embedder.embed_query(text)
            polarity = self._detect_polarity(text)

            # Geçmiş Benzer İlkeler
            hits = self.client.query_points(
                collection_name=LegalConfig.MEMORY_COLLECTIONS["principle"],
                query=vec,
                limit=10,
            ...
            for h in hits:
                ...
                if p != polarity and p != "BELIRSIZ" and polarity != "BELIRSIZ":
                    conflict = True
            ...
            # --- 2. ZAMAN & DOSYA İSMİ DETAYI (V76) ---
            yearly_stats = {}
            for c in cluster_data:
                ts = c.get("timestamp", time.time())
                year = datetime.fromtimestamp(ts).year
                decision = c.get("decision", "KABUL")
                source = c.get("source", "Bilinmeyen Dosya")  # Dosya ismini al

                # Trend Dictionary (Eski logic korunuyor)
            ...
            # Trend'i kaydetme formatına çevir
            trend_dict = {}
            ...
            # --- 3. ANALİTİK LOGLAMA (V76 GÜNCEL) ---
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İLKE KONSOLİDASYONU (ID: {str(uuid.uuid4())[:8]})")
            print("-" * 60)
            print(f"🔹 İLKE: '{text[:80]}...'")
            print(f"🔹 ALAN: {domain}")
            print(f"🔹 ANA YÖN: {polarity}")
            print(f"🔹 ÇELİŞKİ DURUMU: {'⚠️ VAR' if conflict else '✅ YOK (Tutarlı)'}")
            print(f"🔹 EĞİLİM: LEHINE %{lehine_pct:.1f} | ALEYHINE %{aleyhine_pct:.1f}")
            print(f"🔹 KAYNAK DOSYA DÖKÜMÜ:")

            for yr in sorted(yearly_stats.keys()):
                d = yearly_stats[yr]
                print(f"   📅 {yr}:")
                if d['KABUL']:
                    print(f"      ✅ LEHINE ({len(d['KABUL'])} Adet):")
                    for s in d['KABUL']: print(f"         - {s}")
                if d['RED']:
                    print(f"      ❌ ALEYHINE ({len(d['RED'])} Adet):")
            ...
            payload = {
                ...
                "generated_by": "consolidation_v76",
                "timestamp": time.time()
            }
    ...
    # --- V76: DATA EXTRACTION UPDATE ---
    def consolidate_principles_v73(self):
        print("\n🔥 İÇTİHAT MİMARI: İlke-Temelli Konsolidasyon (V76: Detailed Analytics)...")
        try:
            points, _ = self.client.scroll(
                collection_name=LegalConfig.MEMORY_COLLECTIONS["decision"],
                limit=150
                ...
                if (p.payload.get('doc_type') == 'EMSAL KARAR' and
                    ...
                    if p.payload.get('decision') == 'KABUL':
                        candidates.append({
                            ...
                            "id": p.id,
                            "source": p.payload.get('source', 'Bilinmeyen'),  # V76: Dosya adını taşı
...
"""
                ...
                if principle_match:
                    ...
                    # V76 Fonksiyonunu Çağır
                    self._save_principle_v76(principle_text, conf, source_ids, domain_text, cluster)
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V76: Detailed Analytics):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V76: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V76.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V76 (Detailed Traceability & Logging)...")
