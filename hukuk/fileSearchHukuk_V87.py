...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V87: PERSONA CONFLICT ENGINE)
# ==================================================
...
"""
    ...
    # --- V87 YENİ: PERSONA SİNYAL OLUŞTURUCU (DETERMINISTIC) ---
    def _derive_persona_signals(self, analysis_data, item_data):
        """
        Sayısal verilerden persona duruşlarını (Stance, Direction) türetir.
        """
        # HAKİM: Rasyonel ve puana dayalı
        ...
        # SAVCI: Kamu düzeni ve Risk odaklı (İddia Makamı)
        # Savcı trend yukarıysa ve çelişki yoksa güçlü kabul eder.
        # Çelişki varsa risk görür ve reddeder (veya soruşturma ister).
        prosecutor_dir = "acceptance"
        if item_data['conflict'] or item_data['trend_dir'] == 'down':
            prosecutor_dir = "rejection"  # Risk varsa aleyhe döner

        prosecutor = {
            "stance": "strong",  # Savcı genelde nettir
    ...
    # --- V87 YENİ: ÇELİŞKİ ANALİZ MOTORU ---
...
"""
    ...
    def recall_principles(self, query_text):
        try:
            ...
            for item in sorted_hits:
                ...
                # 3. V87: Persona Sinyalleri & Çatışma Analizi (Deterministik)
                ...
                if self.last_recalled_query != query_text:
                    print("\n" + "=" * 70)
                    print(f"📊 [RİSK, PERSONA VE ÇATIŞMA ANALİZİ] (V87: Core Engine)")
                    print(f"   🎯 Sorgu Alanı: {query_domain} | 🎲 Skor: %{analysis['success_probability']}")
                    print("-" * 70)

                    # Yan Yana Görüş Tablosu (Simüle)
                    ...
                    # Çatışma Raporu
                    print(f"⚠️ YARGISAL ÇELİŞKİ ANALİZİ:")
                    print(
                        f"   🔥 Skor: {conflict_analysis['conflict_score']}/100 ({conflict_analysis['conflict_level']})")
                    for reason in conflict_analysis['summary']:
                        print(f"   🔴 {reason}")

                    print("-" * 70)
                    print(f"   ⚖️ HAKİM GÖRÜŞÜ: {judicial_text[:100]}...")
                    print(f"   🏛️ SAVCI GÖRÜŞÜ: {prosecutor_text[:100]}...")
                    print(f"   🛑 RED SENARYOSU: {rejection_text[:100]}...")
                    print("=" * 70 + "\n")
                ...
                memory_text += f"  (Teknik Skor: %{analysis['success_probability']} - {conflict_analysis['conflict_level']} Çatışma)\n"
    ...
    def _save_principle_v79(self, text, confidence, source_ids, domain, cluster_data):
        try:
            ...
            print("\n" + "=" * 60)
            print(f"📊 [ANALİZ LOGU] İÇTİHAT EVRİM RAPORU (V87)")
            print("-" * 60)
            ...
            payload = {
                ...
                "generated_by": "consolidation_v87",
                "timestamp": time.time(),
                "created_at": datetime.now().isoformat()
            }
    ...
    def consolidate_principles_v79(self):
        print("\n🔥 İÇTİHAT MİMARI: Artımlı Konsolidasyon (V87: Conflict Engine)...")
        try:
            time_filter = Filter(
                must=[
                    FieldCondition(
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V87: Persona Conflict Engine):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V87: Full Analysis)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V87.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V87 (Conflict Engine & Multi-Persona Analysis)...")
