...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V106: White-Label Product):")
        valid_docs = []
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V106: Final Output)...")
...
# ==================================================
# 9️⃣ V105-V106 YENİ ARAÇLAR (BRANDED PDF & REASONING)
# ==================================================
class WhiteLabelConfig:
    """Kurumsal kimlik ayarları (V106)"""
...
class JudgeReasoningGenerator:
    """V106: Audit loglardan resmi gerekçe metni üretir"""
    ...
    def generate(self, audit_logs):
        ...
        for log in logs_list:
            if "description" in log: summary_lines.append(f"- {log['description']}")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, audit_data=None, filename="Hukuki_Rapor_V106.pdf", llm=None):
        # V106: Branding Config
        branding = WhiteLabelConfig(
            firm_name="LEGAL OS CORP",  # Burası müşteriye göre değişir
            footer_text="Gizli ve Ozeldir - Otomatik Analiz Raporu",
            color=(0, 51, 102)  # Kurumsal Lacivert
        )

        pdf = BrandedPDFGenerator(branding)

        # KAPAK SAYFASI (CorporateCover - V105)
        CorporateCover.add(pdf, audit_data.get("case_id", "N/A") if audit_data else "N/A", "V106")
        ...
        if audit_data:
            ...
            if llm:
                ...
                pdf.multi_cell(w=pdf.epw, h=6, text=clean(judge_text))

        try:
            pdf.output(filename);
            print(f"\n📄 Kurumsal Rapor (White-Label) Hazır: {filename}")
        except:
            pass

    @staticmethod
    def add_audit_log_section(pdf, audit_data):
        # V105'teki fonksiyonun aynısı (yer kazanmak için tekrar yazmıyorum ama class içinde durmalı)
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V106 (White-Label & Reasoning)...")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # V106: LLM'i reporter'a gönderiyoruz (Gerekçe yazımı için)
                audit_dump = {}
                ...
                # LLM instance'ını hafızadan al
                llm_instance = self.memory_manager.recommendation_engine.llm if self.memory_manager else None

                self.reporter.create_report(story, valid_docs, full_advice, audit_dump, "Hukuki_Rapor_V106.pdf",
                                            llm=llm_instance)
