...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V70: DISCOVERY ENGINE)
# ==================================================
class LegalMemoryManager:
    ...
    # --- V70 GÜNCELLEMESİ: KEŞİF BONUSU ---
    def calculate_memory_consensus(self, source_name, current_decision, vector_score):
        """
        Dönüş: Çarpan (float)
        Mantık:
        1. Geçmişi varsa: Güven oranına göre artır/azalt.
        2. Geçmişi yoksa VE Vektör Skoru yüksekse: 'Keşif Bonusu' ver.
        """
        try:
            ...
            # --- YENİ MANTIK: KEŞİF BONUSU ---
            if not points:
                # Eğer belge hiç kullanılmamışsa (yeni) ama matematiksel olarak çok uyumluysa (score > 0.80)
                if vector_score > 0.80:
                    return 1.10  # %10 'Hoşgeldin Bonusu'
                return 1.0  # Sıradan yeni belge

            # Eski mantık (Geçmişe dayalı güven)
            match_count = sum(1 for p in points if p.payload.get("decision") == current_decision)
            total = len(points)
            ratio = match_count / total

            if ratio > 0.8: return 1.15  # Güvenilir Eski Belge
            if ratio < 0.2: return 0.85  # Hatalı Olduğu Bilinen Belge
            return 1.0
        except:
            return 1.0
    ...
    def save_decision(self, query, doc_name, decision, reason, doc_type):
        try:
            ...
            payload = {
                ...
                "decision": decision,
                "reason": reason,
                "doc_type": doc_type,  # Türü kaydet (Anti-Bias için)
                "timestamp": time.time()
            }
            ...
            # Mevzuat değilse çelişki analizi yap
            if doc_type != "MEVZUAT":
...
"""
    ...
    def consolidate_daily_principles(self):
        """
        Günlük Konsolidasyon (Mevzuat Hariç - Anti-Bias)
        """
        print("\n🔄 GÜNLÜK İLKE KONSOLİDASYONU (Anti-Bias Modu)...")
        try:
            points, _ = self.client.scroll(
                collection_name=LegalConfig.MEMORY_COLLECTIONS["decision"],
                limit=100
            ...
            if not points: print("   ℹ️ Yeterli veri yok."); return

            # Mevzuatları filtreleyerek öğren
            sources = [
                p.payload['source']
                for p in points
            ...
            if not sources:
                print("   ℹ️ Öğrenilecek yeni bir içtihat (Emsal Karar) bulunamadı.")
                return

            source_counts = Counter(sources)

            print("   📊 İÇTİHAT RAPORU:")
            for source, count in source_counts.most_common(3):
                if count > 1:
                    print(f"      🔹 {source}: {count} kez başarılı bulundu.")
                    principle_text = f"İSTATİSTİK: '{source}' kaynağı, sistem tarafından {count} kez güvenilir emsal olarak kabul edilmiştir."
                    self._save_principle(principle_text, source="Sistem İstatistiği")
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (V70: Discovery Engine):")
        valid_docs = []
        ...
        for hit in candidates:
            ...
            # AŞAMA 4 GÜNCELLENDİ: Keşif Bonusu Matematiksel Hesabı
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V70: Full Analysis)...")
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V70 (Discovery Engine: Opportunity for New Docs)...")
