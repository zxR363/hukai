...
class LegalUtils:
    @staticmethod
    def force_unlock_db():
        """Başlangıçta kilit dosyası varsa temizler."""
...
# ==================================================
# 3️⃣ HAFIZA YÖNETİCİSİ (V62 ile Aynı)
# ==================================================
class LegalMemoryManager:
    ...
    def _init_memory_collections(self):
        """Hafıza koleksiyonları yoksa oluşturur."""
    ...
    def recall_principles(self, query_text):
        """Mevcut sorguyla ilgili geçmiş ilkeleri hatırlar."""
        try:
    ...
    def save_decision(self, query, doc_name, decision, reason):
        """Verilen kararı hafızaya kaydeder."""
        try:
...
class LegalSearchEngine:
    ...
    def connect_db(self):
        # --- V63 DÜZELTMESİ: BAĞLANTI KONTROLÜ ---
        if self.client is not None:
            # Zaten bağlıysak tekrar bağlanma, True dön
            return True
    ...
    def run_indexing(self):
        ...
        for key, config in self.config.SOURCES.items():
            ...
            if not new_files:
                print(f"      ✅ {config['desc']} güncel ({len(files_on_disk)} dosya).")
                continue
            ...
            for i in range(0, len(all_texts), batch_size):
                batches.append((all_texts[i:i + batch_size], self.config.EMBEDDING_MODEL))
...
METİN: "{story} {topic}"
ANALİZ KURALLARI:
1. "araba", "miras", "kaza" gibi tek kelimelik girdiler [GEÇERLİ] sayılır.
2. Sadece "asdasd", "lkgjdf" gibi rastgele klavye tuşlamaları [GEÇERSİZ] sayılır.
...
Altına belgenin verdiği hukuki sonucu özetleyen, somut veriye dayalı tek cümlelik gerekçe yaz.
"""
...
"""
    ...
    def evaluate_candidates(self, candidates, story, topic, negatives):
        print("\n⚖️  Akıllı Yargıç Değerlendiriyor (Aşama 2: Rol Atama + Hafıza):")
        valid_docs = []
        ...
        for hit in candidates:
            ...
            if is_ok:
                ...
                print(f"{icon} [{type_desc}] {source:<20} | Güven: %{norm_score:.1f} | Rol: {role}")
    ...
    def generate_final_opinion(self, story, topic, context_str):
        print("\n🧑‍⚖️  AVUKAT YAZIYOR (V62: Learning Mode)...")
...
class LegalReporter:
    @staticmethod
    def create_report(user_story, valid_docs, advice_text, filename="Hukuki_Rapor_V63.pdf"):
...
class LegalApp:
    def __init__(self):
        print("🚀 LEGAL SUITE V63 (Learning Mode + Safe Connection)...")
        self.search_engine = LegalSearchEngine()

        # ÖNCE BAĞLANTI (Tek Sefer)
    ...
    def run(self):
        # run_indexing zaten içindeki connect_db kontrolüyle güvenli
